...
* NEW CODE
  SELECT waers
  UP TO 1 ROWS  FROM t001 INTO lwaer
                      WHERE bukrs = bkpf-bukrs ORDER BY PRIMARY KEY.
...
* NEW CODE
  ...
  UP TO 1 ROWS  FROM  TCURC
         WHERE  WAERS  = bkpf-waers ORDER BY PRIMARY KEY.
...
* NEW CODE
  SELECT kurst
  UP TO 1 ROWS  from t003 into lkurst
                      where blart = bkpf-blart ORDER BY PRIMARY KEY.
