...
*ES GT_GRPTAB  SE BUSCARAN TODOS LOS DEBE Y HABER EN CERO PARA BORRARLOS EN gt_outtab
  LOOP AT gt_grptab INTO ls_grptab WHERE dmbtr_s  EQ 0 AND dmbtr_h EQ 0.
    DELETE gt_outtab
...
*&      Form  PROCESA_TABLA
*&---------------------------------------------------------------------*
FORM procesa_tabla  TABLES ti_bkpf.
*
  LOOP AT ti_bkpf INTO gs_bkpf.
...
*
      SELECT buzei hkont sgtxt dmbtr shkzg
         ...
         FROM bseg WHERE bukrs EQ gs_bkpf-bukrs
      ...
      IF gt_bseg[] IS INITIAL.
        SELECT buzei hkont sgtxt dmbtr shkzg
               INTO CORRESPONDING FIELDS OF TABLE gt_bseg
           FROM bseg_add WHERE bukrs EQ gs_bkpf-bukrs
                           AND belnr EQ gs_bkpf-belnr
                           AND gjahr EQ gs_bkpf-gjahr
...
*       DESCRIPCION CUENTA.
        ...
        CLEAR gs_tabtot.
*
        IF p_mov IS INITIAL AND gs_outtab-belnr IS NOT INITIAL AND
           gs_outtab-dmbtr_h EQ 0 AND gs_outtab-dmbtr_s EQ 0.
* no graba
        ELSE.
