...
* NEW CODE
  SELECT *
 INTO CORRESPONDING FIELDS OF TABLE t_payr
    FROM payr
     WHERE ichec EQ ''
       AND zbukr EQ bukrs
       AND hbkid EQ hbkid
       AND hktid EQ hktid
       AND chect IN psel ORDER BY PRIMARY KEY.
...
*Begin of change: ReSQ Correction for MODIFY on an unsorted Internal Table 27/12/2019 EY_DES01 ECDK916976*
    SORT t_cta.
*End of change: ReSQ Correction for MODIFY on an unsorted Internal Table 27/12/2019 EY_DES01 ECDK916976*
...
* BEGIN. 07-07-2026 - ATC - ATC-03
* OLD CODE
*        SELECT * INTO CORRESPONDING FIELDS OF  t_bsis
*          FROM bsis client specified
*               up to 1 rows
*           WHERE mandt eq sy-mandt
*             and bukrs EQ t_payr-zbukr
...
* NEW CODE
        SELECT *
 INTO CORRESPONDING FIELDS OF  t_bsis
          FROM bsis client specified
               up to 1 rows
           WHERE mandt eq sy-mandt
             and bukrs EQ t_payr-zbukr
             AND hkont EQ t_payr-ubhkt
             AND gjahr EQ t_payr-gjahr
             AND belnr EQ t_payr-vblnr ORDER BY PRIMARY KEY.
...
* NEW CODE
  SELECT bldat
  UP TO 1 ROWS  INTO p_g_bldat
          FROM bkpf
        WHERE  bukrs EQ p_bukrs
          AND  belnr EQ p_t_payr_vblnr
          AND  gjahr EQ p_t_bsis_gjahr ORDER BY PRIMARY KEY.
  ...
  UP TO 1 ROWS  INTO p_emis
   ...
   WHERE bukrs EQ p_bukrs
     ...
     AND ( buzei EQ 1 OR
           buzei EQ 2 )
     AND gjahr EQ p_gjahr
     AND zzmot_emis NE ' ' ORDER BY PRIMARY KEY.
...
* PYV
  ...
  SELECT * INTO CORRESPONDING FIELDS OF TABLE  t_bseg
    ...
    AND   shkzg   EQ  'H'
*Begin of change: ReSQ Correction for Addition ORDER BY PRIMARY KEY 27/12/2019 EY_DES01 ECDK916976*
*    AND   zuonr   EQ  p_chect.
    AND   zuonr   EQ  p_chect ORDER BY PRIMARY KEY.
*End of change: ReSQ Correction for Addition ORDER BY PRIMARY KEY 27/12/2019 EY_DES01 ECDK916976*
...
* NEW CODE
      ...
      UP TO 1 ROWS  INTO p_bldat
        ...
        WHERE  bukrs EQ t_bseg-bukrs
          AND  belnr EQ t_bseg-belnr
          AND  gjahr EQ t_bseg-gjahr ORDER BY PRIMARY KEY.
...
* NEW CODE
      ...
      UP TO 1 ROWS  INTO p_bldat
        ...
        WHERE  bukrs EQ bukrs
          AND  belnr EQ t_bseg-belnr
          AND  gjahr EQ it_payr-gjahr ORDER BY PRIMARY KEY.
