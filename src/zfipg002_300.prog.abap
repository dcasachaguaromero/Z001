...
* NEW CODE
 ...
 INTO TABLE lt_acc
     FROM zfitr009
      WHERE usnam = sy-uname ORDER BY PRIMARY KEY.
...
* NEW CODE
  SELECT puerto
  UP TO 1 ROWS  INTO lv_puerto
         FROM zws_puerto WHERE sociedad EQ bukrs
                           AND programa EQ 'ZFIPG002'
                           AND estado   EQ 'H' ORDER BY PRIMARY KEY.
...
* NEW CODE
              SELECT stcd1 name1
              UP TO 1 ROWS  INTO (l_stcd1, lv_name )
                     FROM lfa1 WHERE lifnr EQ int_tabla3-lifnr ORDER BY PRIMARY KEY.
  ...
  SELECT bukrs belnr gjahr buzei koart bschl
    ...
    INTO CORRESPONDING FIELDS OF TABLE lt_buztab
         FROM bseg WHERE bukrs = bukrs
                     AND belnr = p_intabla3-belnr
                     AND gjahr = p_intabla3-gjahr
                     AND buzei = p_intabla3-buzei ORDER BY PRIMARY KEY.
