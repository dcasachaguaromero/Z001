...
* prepare authorization checks -----------------------------------------
  IF hlp_laufk NE '*'.
    ...
    ELSE.                              "Prüfung für HR-Bestände
      autha-repid = 'ZRFFOUS_C'.          "checks for HR-data
...
* END. 07-07-2026 - ATC - ATC-01
      ...
      ENDIF.
      tab_t042z = t042z.
      IF tab_t042z-progn EQ 'ZRFFOUS_C'
...
* END. 07-07-2026 - ATC - ATC-01
  IF bkpf-awsys EQ t000-logsys OR bkpf-awsys IS INITIAL.
    ...
    ELSEIF l_import_tab-parameter EQ 'IV_DUEDT'.
...
* not valid payment methods from T042Z ---------------------------------
  ...
  LOOP AT err_t042z.
    ...
    AT END OF land1.
      ...
      IF sy-batch EQ space.
        MESSAGE i282 WITH hlp_zwels err_t042z-land1 'ZRFFOUS_C'.
      ENDIF.
      fimsg-msgv1 = hlp_zwels.
      fimsg-msgv2 = err_t042z-land1.
      fimsg-msgv3 = 'ZRFFOUS_C'.
