...
* NEW CODE
      SELECT *
      UP TO 1 ROWS  FROM vbrk WHERE vbeln EQ wa_success-bill_doc ORDER BY PRIMARY KEY.
...
* NEW CODE
      ...
      UP TO 1 ROWS   INTO  wa_zcabpedext
        FROM  zcabpedext  WHERE znum_doc_core = numcore ORDER BY PRIMARY KEY.
...
* NEW CODE
      ...
      UP TO 1 ROWS    INTO vbfa-vbeln
         ...
         WHERE vbelv   EQ p_saldoc
           AND vbtyp_n EQ 'M'
           AND vbtyp_v EQ 'C'
           AND vbeln   EQ wa_success-bill_doc ORDER BY PRIMARY KEY.
...
* NEW CODE
    SELECT *
    UP TO 1 ROWS   INTO wa_zcabpedext
      FROM zcabpedext WHERE znum_doc_core EQ p_numdoc ORDER BY PRIMARY KEY.
...
* NEW CODE
    ...
    UP TO 1 ROWS  INTO lv_dea
      FROM ztdea   WHERE blart = wa_zcabpedext-zblart ORDER BY PRIMARY KEY.
...
* NEW CODE
  SELECT fkart
  UP TO 1 ROWS  INTO l_fkart
         FROM vbrk WHERE vbeln EQ p_vbeln ORDER BY PRIMARY KEY.
...
* NEW CODE
  ...
  UP TO 1 ROWS 
    INTO wa_zcabpedext
    FROM zcabpedext
    WHERE znum_doc_core EQ p_numdoc ORDER BY PRIMARY KEY.
