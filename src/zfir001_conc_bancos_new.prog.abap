...
* NEW CODE
  SELECT *
 FROM bsas APPENDING CORRESPONDING FIELDS OF TABLE it_bsis
             WHERE bukrs = p_bukrs
             AND   hkont IN s_saknr
             AND   augdt > p_budat
...
* NEW CODE
      SELECT *
      UP TO 1 ROWS  FROM t012k WHERE bukrs = it_bsis-bukrs
                                 AND   hbkid = it_bsis-hbkid ORDER BY PRIMARY KEY.
...
* NEW CODE
      ...
      UP TO 1 ROWS  FROM  payr
           ...
           AND   hktid  EQ t012k-hktid
  "         AND   rzawe  EQ 'C'
           AND   chect  EQ chect_aux ORDER BY PRIMARY KEY.
...
* NEW CODE
      SELECT stcd1 name1
      UP TO 1 ROWS  INTO (it_bsis-stcd1, it_bsis-name1) FROM lfa1
                     WHERE lifnr  EQ payr-lifnr ORDER BY PRIMARY KEY.
...
* NEW CODE
      SELECT *
      UP TO 1 ROWS  FROM  bseg WHERE bukrs  = it_bsis-bukrs
                                         AND  belnr = it_bsis-belnr
                                         AND  gjahr = it_bsis-gjahr
...
* NEW CODE
        SELECT txt50
        UP TO 1 ROWS  INTO txt50 FROM skat
            WHERE ktopl = p_pcta
              AND saknr = it_bsis-hkont
              AND spras = sy-langu ORDER BY PRIMARY KEY.
