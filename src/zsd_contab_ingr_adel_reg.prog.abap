...
*&---------------------------------------------------------------------*
*&---------------------------------------------------------------------*
*& Report  ZFI_Contabilizacion Ingresos Adelantados regulariza
*&                             cuotas pendientes
*&---------------------------------------------------------------------*
*&.
*&..
...
REPORT  zsd_contab_ingr_adel_reg.
...
*
    SELECT k~bukrs k~vbeln k~fkdat k~fkart k~fkdat
           k~zcuotas_fin k~knumv k~waerk k~kunrg k~zcuotas k~znum_cuotas p~kvgr4 p~posnr k~xblnr
...
*End of change: ReSQ Correction for Addition ORDER BY PRIMARY KEY 23/12/2019 EY_DES02 ECDK917080 *
      ...
      ENDLOOP.
      "
      IF NOT <fs_data>-zcuotas LE <fs_data>-znum_cuotas.
        CONTINUE.
      ELSE.
        cont = <fs_data>-zcuotas + 1.
        new_date = <fs_data>-fkdat.
        "
        gv_ncuotas = 0.
        WHILE cont <= <fs_data>-znum_cuotas.
          PERFORM add_month USING 1 new_date CHANGING new_date.
          IF new_date(6) < p_budat(6).
            gv_ncuotas = gv_ncuotas + 1.
          ENDIF.
          ADD 1 TO cont.
        ENDWHILE.
        IF gv_ncuotas = 0. CONTINUE. ENDIF.
        ...
        wa_out-cuota_contab = gv_ncuotas.
        " Fecha de cuota o contabilizacion
        wa_out-fecha = p_budat.
        "
        APPEND wa_out TO t_out.
        "
        l_sum_fact = l_sum_fact + <fs_data>-kbetr.
      ENDIF.
      "
      AT END OF vbeln.
        IF wa_out-vbeln IS NOT INITIAL.
          ...
          wa_out-icon = gv_icon.
          " Valor Cuota
          wa_out-valor = l_sum_fact * gv_ncuotas.
          "
...
** Declaration for Table Selection settings
    ...
    gr_select = o_alv->get_selections( ).
    IF gr_select IS NOT INITIAL.
      gr_select->set_selection_mode(
    ...
    "
    "CALL METHOD o_alv->GET_SELECTions
    "
    o_alv->display( ).
    "
...
*End of change: ReSQ Correction for INDEX READ on an unsorted Internal TABLE 23/12/2019 EY_DES02 ECDK917080 *
    LOOP AT lt_row INTO ls_row .
      ...
      READ TABLE t_out_res  INTO wa_out INDEX ls_row.
      CHECK  sy-subrc EQ 0.
      l_vbeln = wa_out-vbeln.
      gv_ncuotas = wa_out-cuota_contab.
      CLEAR wa_out.
      LOOP AT t_out_res INTO wa_out WHERE vbeln EQ l_vbeln.
        l_index = sy-tabix.
...
*End of change: ReSQ Correction for Addition ORDER BY PRIMARY KEY 23/12/2019 EY_DES02 ECDK917080 *
        ...
        LOOP AT t_konv INTO wa_konv WHERE knumv EQ vbrk-knumv
          ...
          CASE wa_konv-kschl.
            WHEN 'ZCUO'.
              ls_res_cond_cta-newko = '4111170039'.
              ls_res_cond_cta-newbs = '50'.
            WHEN 'ZDC1'.
              ls_res_cond_cta-newko = '4111170041'.
              ls_res_cond_cta-newbs = '40'.
            WHEN 'ZDC2'.
              ls_res_cond_cta-newko = '4111170040'.
              ls_res_cond_cta-newbs = '40'..
            WHEN 'ZDC3'.
              ls_res_cond_cta-newko = '4111170042'.
              ls_res_cond_cta-newbs = '40'.
            WHEN 'ZDC4'.
              ls_res_cond_cta-newko = '4111170043'.
              ls_res_cond_cta-newbs = '40'.
            WHEN 'ZDC5'.
              ls_res_cond_cta-newko = '4111170044'.
              ls_res_cond_cta-newbs = '40'.
            WHEN 'ZRE1'.
              " V1 - Ini - JSHANNON - 2017.10.25
...
**------------------------------------------------------------
          ...
          ENDIF.
          "
          ls_res_cond_cta-kbetr = ls_res_cond_cta-kbetr * gv_ncuotas.
          "
...
**-----------------------------------------------------
        ...
        IF l_exito EQ abap_true.
          ...
          wa_out-text = l_text.
          "
          wa_out-cuota_contab = wa_out-zcuotas + wa_out-cuota_contab.
          "
...
*      -->P_FKDAT  text
*----------------------------------------------------------------------*
FORM add_month  USING p_cont TYPE i
                      p_date TYPE sy-datum
                CHANGING new_date TYPE sy-datum.
