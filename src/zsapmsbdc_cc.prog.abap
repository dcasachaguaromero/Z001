...
* NEW CODE
    ...
    UP TO 1 ROWS  FROM bdcuseop WHERE bdcuser = sy-uname
                                    AND tid     = 'SM35' ORDER BY PRIMARY KEY.
...
* BEGIN. 07-07-2026 - ATC - ATC-01
* OLD CODE
*    SELECT SINGLE * FROM apql WHERE qid = mtab-qid.     
*
* NEW CODE
    SELECT *
    UP TO 1 ROWS  FROM apql WHERE qid = mtab-qid ORDER BY PRIMARY KEY.     

    ENDSELECT.
* END. 07-07-2026 - ATC - ATC-01"#EC CI_NOFIELD
    IF sy-subrc = 0.
      log-retc = 'PROT'.
      EXIT.
    ENDIF.
...
* NEW CODE
  SELECT *
 FROM apqd
  WHERE qid        =     mtab-qid ORDER BY PRIMARY KEY.
...
* NEW CODE
  SELECT *
  UP TO 1 ROWS  FROM usr02
    WHERE bname = auth_user ORDER BY PRIMARY KEY.
...
* NEW CODE
  SELECT *
  UP TO 1 ROWS  FROM apqd WHERE qid = mtab-qid AND block = 1 ORDER BY PRIMARY KEY.
...
*           WHERE vbfunc = 'BDC_END_VB'.                 
*
* NEW CODE
  SELECT *
 FROM vbmod
           INTO TABLE itab_vbmod
           WHERE vbfunc = 'BDC_END_VB' ORDER BY PRIMARY KEY.                 

* END. 07-07-2026 - ATC - ATC-03"#EC CI_NOFIELD
...
* NEW CODE
 ...
 FROM vbdata
                        INTO TABLE lt_vbdata
                        WHERE vbkey = wa_vbmod-vbkey
                          AND vbmodcnt = wa_vbmod-vbmodcnt ORDER BY PRIMARY KEY.
...
* NEW CODE
      SELECT *
      UP TO 1 ROWS  FROM vbhdr
                      INTO wa_vbhdr
                      WHERE vbkey = wa_vbmod-vbkey ORDER BY PRIMARY KEY.
