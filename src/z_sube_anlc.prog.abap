...
*&
*&---------------------------------------------------------------------*
*&Rescata Tabla ANLC Original Respaldada
*&
*&---------------------------------------------------------------------*
...
data : begin of t_texto occurs 0,
       ...
       begin of t_ANLC occurs 0.
        include structure ANLC.
data :    box   type c,
       end of t_ANLC,
       err_ANLC like ANLC occurs 0 with header line,
       ERROR    TYPE C.
* Variables ALV
TYPE-POOLS slis.
...
DATA : MAX_AFBLPE LIKE ANLC-AFBLPE,
       MAX_AFBANZ LIKE ANLC-AFBANZ.
...
FORM user_command USING r_ucomm     LIKE sy-ucomm
  ...
  case r_ucomm.
    when 'FC01'.
      refresh err_ANLC.
      LOOP AT T_ANLC WHERE BOX EQ 'X'.
        MOVE SY-TABIX TO L_TABIX.
        move-corresponding t_ANLC to ANLC.
*
* PGR CARGA EN AREA ALTERNATIVA
        IF NOT P_AF_ALT IS INITIAL.
          move P_AF_ALT to anlc-afabe.
        ENDIF.
***********************************
*        insert ANLC.
* PGR
        ...
        MODIFY ANLC.
        if sy-subrc ne 0.
          append ANLC to err_ANLC.
        endif.
        T_ANLC-BOX = ' '.
        MODIFY T_ANLC INDEX L_TABIX.
      ENDLOOP.
      IF SY-SUBRC NE 0.
...
*    ls_line-info = 'Datos de tabla ANLC Respaldados'.
*  ELSE.
*    ls_line-info = 'Datos con ERROR de tabla ANLC Respaldados'.
*  ENDIF.
...
***********************************
    ...
    MODIFY ANLC.
    if sy-subrc ne 0.
      write:/ 'Error al Grabar' , anlc-anln1, anlc-anln2.
    endif.
  ENDLOOP.
*
