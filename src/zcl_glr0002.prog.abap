...
* NEW CODE
  SELECT pathintern
  UP TO 1 ROWS  INTO path_o FROM ztab_pro_mas WHERE tip_lib =
  'VENTAS' ORDER BY PRIMARY KEY.
...
*
* NEW CODE
  SELECT znom_cli_fact zrut_cli_pagador
  UP TO 1 ROWS 
    INTO (tabla-znom_cli_fact, tabla-zrut_cli_pagador)
    FROM zfac_anex
    WHERE bukrs EQ bkpf-bukrs
      AND belnr EQ bkpf-belnr
      AND gjahr EQ bkpf-gjahr ORDER BY PRIMARY KEY.
...
* NEW CODE
    SELECT *
    UP TO 1 ROWS  FROM zdatsocelec WHERE bukrs EQ br_bukrs-low ORDER BY PRIMARY KEY.
...
* NEW CODE
  SELECT ltext
  UP TO 1 ROWS  INTO res_ventas-nomb FROM t003t WHERE spras EQ
  sy-langu
                                                        AND blart EQ low
