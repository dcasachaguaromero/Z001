...
* NEW CODE
  SELECT fkart zblart
  UP TO 1 ROWS  INTO ( l_fkart, l_zblart )
         FROM vbrk WHERE vbeln EQ objky ORDER BY PRIMARY KEY.
...
* NEW CODE
  SELECT kappl objky kschl spras
    ...
    WHERE kappl EQ 'V3'
      AND objky EQ objky
      AND kschl EQ l_kschl ORDER BY PRIMARY KEY.
...
* NEW CODE
  ...
  UP TO 1 ROWS 
    INTO clas_doc
    FROM zcabpedext
    WHERE factura EQ objky ORDER BY PRIMARY KEY.
...
* NEW CODE
  ...
  UP TO 1 ROWS 
    INTO marca
    FROM ztdea
    WHERE blart EQ clas_doc ORDER BY PRIMARY KEY.
