...
REPORT zrfbibl01_v2 MESSAGE-ID fb.
************************************************************************
...
*---- BBKPF enhancement: CUP and CIG                        "2142438
    IF bbkpf-cup(1) NE nodata.                              "2142438
* ------ BBKPF enhancement for extended exchange rate          "GLE_FXR
    IF bbkpf-kursx_m(1) NE nodata.                             "GLE_FXR
      IF bbkpf-kursx(1) NE nodata.                             "GLE_FXR
...
*------- BBKPF-Erweiterung zu 2.1A: VBUND --
    ...
    ENDIF.                                                     "GLE_FXR
    bbkpf-kursx(1) = nodata.                                   "GLE_FXR
    ENDIF.                                                     "GLE_FXR
    bbkpf-kursx_m(1) = nodata.                                 "GLE_FXR
    ENDIF.                                                  "2142438
    bbkpf-cup(1) = nodata.                                  "2142438
    bbkpf-cig(1) = nodata.                                  "2142438
...
* Felder aus dem Standard) nicht durchgefuehrt werden.
    ...
    IF bbseg-intad ne nodata.                                 "2661564
    IF bbseg-stcd5 NE nodata.                               "2495310
    IF bbseg-j_1tpbupl NE nodata.                             "2120258
    IF bbseg-mndid NE nodata.                                 "1888262
      IF bbseg-boeno NE nodata.                               "1723936
        IF bbseg-avgact_g NE nodata.                          "1593913
...
*end of insertion note 1115584
    ENDIF.
    bbseg-avgact_g(1)        = nodata.                      "1593913
    bbseg-avgact_l(1)        = nodata.                      "1593913
    bbseg-avgact_t(1)        = nodata.                      "1593913
    bbseg-avg_act_g_curr(1)  = nodata.                      "1593913
    bbseg-avg_act_l_curr(1)  = nodata.                      "1593913
    bbseg-avg_Act_t_curr(1)  = nodata.                      "1593913
    ENDIF.
    bbseg-boeno(1) = nodata.
    ENDIF.
    bbseg-mndid(1) = nodata.                                "1888262
    ENDIF.
    bbseg-j_1tpbupl(1) = nodata.                            "2120258
    ENDIF.
    bbseg-stcd5(1) = nodata.                                "2495310
    bbseg-acrobjtype(1) = nodata.                           "2563329
    bbseg-acrobj_id(1) = nodata.                            "2563329
    bbseg-acrsobj_id(1) = nodata.                           "2563329
    bbseg-acritmtype(1) = nodata.                           "2563329
    bbseg-valobjtype(1) = nodata.                           "2657208
    bbseg-valobj_id(1) = nodata.                            "2657208
    bbseg-valsobj_id(1) = nodata.                           "2657208
    ENDIF.                                                  "2661564
    bbseg-intad(1) = nodata.                                "2661564
...
FORM bbtax_erweiterung_pruefen.
  ...
  IF bbtax-sende(1) NE nodata.
    IF bbtax-ctxkrs(1) NE nodata.                           "N2054102
      IF bbtax-txkrs(1) NE nodata.                          "2138584
      IF bbtax-fwbas(1) NE nodata.       "2138584

*------- BBTAX-Erweiterung zu 4.0A: H2STE(16), H3STE(16)
        bbtax-h2ste(1) = nodata.
        bbtax-h3ste(1) = nodata.
      ENDIF.                             "2138584
      bbtax-fwbas(1) = nodata.           "2138584
      bbtax-hwbas(1) = nodata.           "2138584
      bbtax-h2bas(1) = nodata.           "2138584
      bbtax-h3bas(1) = nodata.           "2138584
    ENDIF.                               "2138584
    bbtax-txkrs(1) = nodata.             "2138584
    ENDIF.                                                  "N2054102
    bbtax-ctxkrs(1) = nodata.                               "N2054102
...
*-----------------------------------------------------------------------
    WHEN 'BTAX'.
      ...
      PERFORM bbtax_erweiterung_pruefen.                   "2138584
...
FORM mappe_oeffnen.
  ...
  IF bgr00-usnam(1) = NODATA. "2291261
    bgr00-usnam = SPACE.      "2291261
  ENDIF.                      "2291261
  ...
  IF sy-subrc = 1.
    IF bgr00-usnam = space.
      PERFORM log_msg USING c_msgid 'I' '009'
                          group_count space space space.
    ELSE.
      PERFORM log_msg USING sy-msgid sy-msgty sy-msgno
...
FORM move_bselk_to_save_ftclear.
  ...
  ENDIF.
  IF NOT bselk-agums_long IS INITIAL                           "n2073200
     AND bselk-agums_long(1) NE nodata.                        "n2073200
    save_ftclear-agums = bselk-agums_long.                     "n2073200
  ELSEIF bselk-agums(1) NE nodata.                             "n2073200
    save_ftclear-agums = bselk-agums.                          "n2073200
  ENDIF.                                                       "n2073200
  IF bselk-avsid(1) NE nodata.         "Avis
...
*------- Ausstieg bei Direct Input wg. Performance --------------------
    ...
    ELSE.
      ...
      if bbseg-txjcd(1) ne NODATA.                            "N2040171
...
*------- Datei öffnen -----------------------------------------------
  ...
  DATA: lv_ds_name TYPE char75,
        lv_file_a  TYPE char12,
        lv_file    TYPE string. "JOROZCO 21.01.2020

* begin "n1592941

  CONCATENATE '/tmp/' ds_name INTO lv_ds_name."jorozco 21.01.2020
  lv_file_a = lv_ds_name(12).
  CALL FUNCTION 'FILE_VALIDATE_NAME'
    EXPORTING
      logical_filename  = gc_filename
      parameter_1       = sy-cprog
    CHANGING
*      physical_filename = ds_name "JOROZCO 21.01.2020
      physical_filename = lv_ds_name "JOROZCO 21.01.2020
    EXCEPTIONS
      OTHERS            = 1.
...
*    OPEN DATASET ds_name IN TEXT MODE   "jorozco 21.01.2020
    OPEN DATASET lv_ds_name IN TEXT MODE "jorozco 21.01.2020
      ENCODING UTF-8
      FOR INPUT
      SKIPPING BYTE-ORDER MARK
      IGNORING CONVERSION ERRORS.
  ELSE.
*    OPEN DATASET ds_name IN TEXT MODE ENCODING NON-UNICODE"jorozco 21.01.2020
    OPEN DATASET lv_ds_name IN TEXT MODE ENCODING NON-UNICODE"jorozco 21.01.2020
...
*    READ DATASET ds_name INTO tfile-rec.                   "N1847840
    READ DATASET lv_ds_name INTO tfile-rec.                   "N1847840

*------- End of File erreicht ? Exit Do-Schleife -------------------
    IF sy-subrc NE 0.
      EXIT.
    ENDIF.
    REPLACE ALL OCCURRENCES OF lv_file_a IN tfile-rec WITH ds_name. "jorozco 21.01.2020
*    REPLACE ALL OCCURRENCES OF lv_ds_name IN tfile-rec WITH ds_name. "jorozco 21.01.2020
