...
* NEW CODE
  SELECT *
 FROM FEBKO WHERE ANWND = ANWND AND        "note 620244
                            ASTAT <> '8' AND          "hw380862
                            AZDAT IN S_AZDAT ORDER BY PRIMARY KEY.
...
* NEW CODE
      SELECT *
 from febep into l_febep                      "note 596474
                         where kukey = febko-kukey ORDER BY PRIMARY KEY.         

* END. 07-07-2026 - ATC - ATC-03"note 596474
...
* NEW CODE
  SELECT *
 FROM FEBKO  WHERE KUKEY IN S_KUKEY AND ANWND = '0001' ORDER BY PRIMARY KEY.
