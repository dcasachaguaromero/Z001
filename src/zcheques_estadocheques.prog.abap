...
REPORT  ZCHEQUES_ESTADOCHEQUES.

type-pools: slis.
tables : payr, bsas, bsis.
...
SELECT *
 FROM BSIS
  where bukrs in s_bukrs
        and hkont in s_hkont ORDER BY PRIMARY KEY.
...
SELECT *
 FROM BSAS
  where bukrs in s_bukrs
        and hkont in s_hkont
        and xragl <> 'X' ORDER BY PRIMARY KEY.

* END. 07-07-2026 - ATC - ATC-03

  check bsas-augbl <> bsas-belnr.
  CLEAR it_spins.
  it_spins-ptipo_pago = 'C'.
*  it_spins-pfec_emision = payr-ZALDT.
* BEGIN. 07-07-2026 - ATC - ATC-01
