...
* NEW CODE
    ...
    UP TO 1 ROWS  FROM t001
           WHERE bukrs = cia ORDER BY PRIMARY KEY.
...
* NEW CODE
      SELECT *
      UP TO 1 ROWS  FROM reguh WHERE identif_pago = to_file-refcli ORDER BY PRIMARY KEY.
...
* NEW CODE
      ...
      UP TO 1 ROWS  FROM zestadosbanco
                     WHERE banco  = banco
                       AND codban = to_file-estpag ORDER BY PRIMARY KEY.
