...
* NEW CODE
  SELECT *
 FROM zfipg201 INTO CORRESPONDING FIELDS OF TABLE int_motban WHERE bukrs  = bukrs ORDER BY PRIMARY KEY.
...
* NEW CODE
 ...
 FROM bsik WHERE bukrs = bukrs
    ...
    AND   zzmot_emis  = p_xzzmot
    AND   hbkid =  p_hbkid
    AND   xref1 IN p_xref1 ORDER BY PRIMARY KEY.
...
* NEW CODE
        ...
        UP TO 1 ROWS  INTO  int_tabla-descr
                              FROM  zmot_emis
                              WHERE bukrs = bukrs
                              AND   zzmot_emis  =  zzmot_emis_p ORDER BY PRIMARY KEY.
