...
REPORT  zfilb003  MESSAGE-ID    icc_cl
...
DATA: S_TEXTO(255) TYPE C. " se guarda el texto
DATA: G_FLENGTH TYPE I. "Devuelve el total de bytes transferidos.
DATA : TXTDR(30), TXT1(30),TXT2(15),TXT3(15) ,TXT4(30) ,TXT5(15) ,TXT6(15) ,TXT7(15) ,TXT8(15), TXT9(15), TXT10(15).
DATA:  V_HORIZ_TAB(1) TYPE C.
DATA : V_NEWLINE(1) TYPE C.
DATA G_LINEAS TYPE I.
DATA G_PRIMERO TYPE I.
DATA G_TOTLAUFD TYPE I.
CONSTANTS SCPAG TYPE I VALUE 55.
CONSTANTS GC_01(02) TYPE C VALUE '01'.
          .
DATA : PATH_FILE     LIKE RLGRAP-FILENAME.
DATA:  L_CALLBACK_HTML_TOP_OF_PAGE  TYPE SLIS_FORMNAME VALUE 'HTML_TOP_OF_PAGE'.
...
DATA ADDRS TYPE ADRC-STREET.
DATA g_line           TYPE i VALUE 50.
...
TYPES: BEGIN OF out,                                    " structure
            blart       LIKE bsik-blart,
            LTEXT       like T003T-LTEXT,          " nombre  de documento
*            blart       type   c,
            ...
            dmbtr       LIKE  with_item-wt_qsshh, " bsik-dmbtr,          " invoice amount
            dmbt2       LIKE  with_item-wt_qsshh, " bsik-dmbtr,          "invoice amount/period
            dmbt3       LIKE  with_item-wt_qsshh, "bsik-dmbtr,          "inv am/per to certify
            wt_qsshh    LIKE with_item-wt_qsshh,  " wt base amount
            wt_qbshh    LIKE with_item-wt_qbshh,  " wt amount
            wt_qbsh2    LIKE with_item-wt_qbshh,  " wt amount/period
            wt_qbsh3    LIKE with_item-wt_qbshh,  " wt amount/period
                                                  " to certify
            qsatz     LIKE   with_item-wt_qbshh,  "with_item-qsatz,        "Tasa
...
*select-OPTIONS: s_qsskz1 for with_item-witht OBLIGATORY DEFAULT 'Q1'.


SELECTION-SCREEN BEGIN OF LINE.
SELECTION-SCREEN COMMENT 1(20) text-099 FOR FIELD s_qsskz1.
SELECTION-SCREEN POSITION POS_LOW.
*PARAMETERS: qkz3 LIKE with_item-wt_withcd OBLIGATORY DEFAULT 'B1'.
*SELECT-OPTIONS: qkz3 FOR with_item-wt_withcd OBLIGATORY DEFAULT 'B1'.
select-OPTIONS: s_qsskz1 for with_item-witht OBLIGATORY DEFAULT 'Q1'.
SELECTION-SCREEN END OF LINE.

SELECTION-SCREEN BEGIN OF LINE.
SELECTION-SCREEN COMMENT 1(10) text-098 FOR FIELD qkz0.
SELECTION-SCREEN POSITION POS_LOW.

*PARAMETERS: qkz0 LIKE with_item-wt_withcd OBLIGATORY DEFAULT 'B1'.
sELECT-OPTIONS: qkz0 FOR with_item-wt_withcd OBLIGATORY DEFAULT 'B1'.

SELECTION-SCREEN END OF LINE.


SELECTION-SCREEN BEGIN OF LINE.
SELECTION-SCREEN COMMENT 1(15) text-100 FOR FIELD ZFILE.
*PARAMETERS: p_def AS CHECKBOX DEFAULT ' '.
...
*SELECTION-SCREEN BEGIN OF BLOCK 1111 WITH FRAME TITLE text-111.
*PARAMETERS: r_clp RADIOBUTTON GROUP g1 DEFAULT 'X'.
**            r_usd RADIOBUTTON GROUP g1.
*SELECTION-SCREEN END OF BLOCK 1111.
...
REFRESH qkz0.
  ...
  MOVE: 'B2' TO qkz0-high,
        'I'    TO qkz0-sign,
        'EQ'   TO qkz0-option.
  APPEND qkz0.

REFRESH s_qsskz1.
  MOVE: 'Q1' TO s_qsskz1-low,
        'I'    TO s_qsskz1-sign,
        'EQ'   TO s_qsskz1-option.

  MOVE: 'Q2' TO s_qsskz1-high,
        'I'    TO s_qsskz1-sign,
        'EQ'   TO s_qsskz1-option.
  APPEND s_qsskz1.
...
*----------------------------------------------
*Moneda Visualización Reporte

*  IF p_def = 'X'.   "Reporte Legal
**    IF r_usd = 'X'.
**      PERFORM submit_report_leg_usd.
**    ELSE.
*      PERFORM submit_report_leg.
**    ENDIF.
**  ELSE.             "No Legal
**    IF r_usd = 'X'.
**      PERFORM submit_report_usd.
**    ENDIF.
*  ENDIF.
*----------------------------------------------
...
*     IF p_def = 'X'. " si se marca legal se genera archivo
...
*      SET PF-STATUS 'ZFULLSCREEN'.
      ...
      USING  g_expa
                  CHANGING
                     gt_sort.
       PERFORM print_final2.
       PERFORM ls_output_abs.
       PERFORM reporte.
     endif.
...
*    se comenta ELSE para no mostrar documentos Nulos
    IF ok EQ 'X'.
...
*        MOVE part_work-dmbtr TO output_list-dmbtr.
        ...
        MOVE with_item-wt_qbshh TO output_list-wt_qbshh.
        MOVE with_item-wt_qsshh TO output_list-wt_qsshh.
...
*
        ...
        ENDIF.
* --- get vendor data --------------------------------------------------
        READ TABLE vend_itab WITH KEY lifnr = part_work-LIFNR.
...
* END. 07-07-2026 - ATC - ATC-01
        ...
        MOVE part_work-cont_null TO output_list-cont_null.
        MOVE part_work-dmbtr TO output_list-dmbtr.
        MOVE part_work-dmbtr TO output_list-WT_QSSHH.
*        output_list-dmbtr = with_item-wt_qsshh - with_item-wt_qbshh.
        MOVE t001-waers TO output_list-waers.
*        MOVE with_item-wt_qbshh TO dmbtr.
...
*hide: output_list-belnr.
    ...
    WRITE: wa-wt_qsshh TO c_wt_qsshh NO-SIGN CURRENCY t001-waers,
           wa-wt_qbshh TO c_wt_qbshh NO-SIGN CURRENCY t001-waers,
           wa-dmbtr    TO c_dmbtr    NO-SIGN CURRENCY t001-waers.
...
*                wa-ort02 UNDER text-412,
                ...
                wa-wt_qsshh NO-SIGN CURRENCY t001-waers UNDER text-406,
**                c_wt_qsshh  UNDER text-406,
*                wa-qsatz  DECIMALS 0   UNDER text-411,
                wa-wt_qbshh NO-SIGN CURRENCY t001-waers UNDER text-407,
**                c_wt_qbshh  UNDER text-407,
                wa-dmbtr    NO-SIGN CURRENCY t001-waers UNDER text-408.
...
*            cont TO total_table-cont
      ...
      WRITE: /'', text-799 UNDER text-404, type_desc UNDER text-398,
                output_list-wt_qsshh  NO-SIGN CURRENCY t001-waers UNDER text-406,
*                c_wt_qsshh  UNDER text-406,
                 output_list-wt_qbshh  NO-SIGN CURRENCY t001-waers  UNDER text-407,
                 output_list-dmbtr  NO-SIGN CURRENCY t001-waers  DECIMALS 0   UNDER text-408.
...
*    write:/1 '|',   total_table-blart, '                        ', '|',
        total_table-cont_null NO-SIGN, '          ' , '|',
        total_table-count NO-SIGN,'          ', '|',
        total_table-wt_qsshh NO-SIGN CURRENCY t001-waers, '|',
        total_table-wt_qbshh NO-SIGN CURRENCY t001-waers, '|',
        total_table-dmbtr NO-SIGN CURRENCY t001-waers, '|'.
...
*            WITH s_busa IN s_busa
            ...
            WITH s_year EQ s_year
            VIA SELECTION-SCREEN.
ENDFORM.                    " submit_report_leg_usd



*&---------------------------------------------------------------------*
*&      Form  submit_report_usd
*&---------------------------------------------------------------------*
*       text
*----------------------------------------------------------------------*
*  -->  p1        text
