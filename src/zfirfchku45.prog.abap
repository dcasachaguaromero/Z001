...
START-OF-SELECTION.
  ...
  ELSE.
    WRITE: / '-----------------------------------------------------------------'.
    WRITE: / '-     CARTOLA NO EXISTE EN TABLA                         '.
    WRITE: / '-----------------------------------------------------------------'.
    WRITE: / 'Clave Breve Cartola                                     :', cartola.
    EXIT.
  ENDIF.
...
*ResQ Comment:Correction not required as Select Single is used 20/12/2019 EY_DES02 ECDK917080 *
  ...
  ELSE.
    zcb_logajustes-estpro = '0'.
    CONCATENATE 'Cheque: ' febep_t-chect ' no valido' INTO zcb_logajustes-obspro.
    zcb_logajustes-belnr  = '          '.
    PERFORM graba_log.
  ENDIF.
ENDFORM.                                                    "REVISAR
...
*----------------------------------------------------------------------*
FORM compensa_ajuste.
  MOVE febep_t-kukey TO cartola.
  MOVE febep_t-esnum TO linea.
  CLEAR bdcdata.
  REFRESH bdcdata.
*&---------------------------------------------------------------------*
...
*&---------------------------------------------------------------------*
FORM graba_log.
  zcb_logajustes-bukrs  = febko-bukrs.
  zcb_logajustes-kukey  = febko-kukey.
