...
*&
*&---------------------------------------------------------------------*
*&Rescata Tabla ANLB Original Respaldada
*&
*&---------------------------------------------------------------------*
...
data : begin of t_texto occurs 0,
       ...
       begin of t_ANLB occurs 0.
        include structure ANLB.
data :    box   type c,
       end of t_ANLB,
       err_ANLB like ANLB occurs 0 with header line,
       ERROR    TYPE C.
* Variables ALV
TYPE-POOLS slis.
...
start-of-selection.
  ...
  perform lee_ANLB.
*Si es Modo Batch actualiza todo directamente.
...
FORM user_command USING r_ucomm     LIKE sy-ucomm
  ...
  case r_ucomm.
    when 'FC01'.
      refresh err_ANLB.
      LOOP AT T_ANLB WHERE BOX EQ 'X'.
        MOVE SY-TABIX TO L_TABIX.
        move-corresponding t_ANLB to ANLB.
*
* PGR CARGA EN AREA ALTERNATIVA
        IF NOT P_AF_ALT IS INITIAL.
          move P_AF_ALT to ANLB-afabe.
        ENDIF.
***********************************
*        insert ANLB.
* PGR
        ...
        MODIFY ANLB.
        if sy-subrc ne 0.
          append ANLB to err_ANLB.
        endif.
        T_ANLB-BOX = ' '.
        MODIFY T_ANLB INDEX L_TABIX.
      ENDLOOP.
      IF SY-SUBRC NE 0.
...
*    ls_line-info = 'Datos de tabla ANLB Respaldados'.
*  ELSE.
*    ls_line-info = 'Datos con ERROR de tabla ANLB Respaldados'.
*  ENDIF.
...
***********************************
    ...
    MODIFY ANLB.
    if sy-subrc ne 0.
      write:/ 'Error al Grabar' , ANLB-anln1, ANLB-anln2.
    endif.
  ENDLOOP.
*
