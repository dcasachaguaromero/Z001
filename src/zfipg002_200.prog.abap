...
MODULE status_0200 OUTPUT.
  ...
  CONCATENATE 'PROCESO-' fecha '-' hora '-' zlsch INTO zfipg002_cab-descr.

* ini - 04-06-2020 - Waldo alarcon - Visionone.
  IF gv_mensaje IS NOT INITIAL.
    LOOP AT SCREEN.
      CHECK screen-group1 EQ 'MSJ'.
...
*End of change: ReSQ Correction for INDEX READ on an unsorted Internal TABLE 20/12/2019 EY_DES02 ECDK917080 *
  READ TABLE int_tabla2 INTO zfipg002_a_est INDEX tabla2-current_line.
...
FORM batchinput_propuesta.
  ...
  DATA: fecha1(8),
        ...
        fecha3(8),
        fecha         LIKE sy-datum,
        nproceso      LIKE zfipg002_cab-nproceso,
        variante1(20),
        variante2(20),
        veces(4)      TYPE n.

  IF zfipg002_cab-descr IS INITIAL.
...
* NEW CODE
    SELECT *
    UP TO 1 ROWS   FROM zfolio_soc  WHERE bukrs  = bukrs ORDER BY PRIMARY KEY.
...
*----------------------------------------------------------------------------------------
    ...
    zfipg002_cab-bukrs       = bukrs.
    zfipg002_cab-nproceso    = nproceso.
    zfipg002_cab-crea_fecha  = sy-datum.
    zfipg002_cab-crea_hora   = sy-uzeit.
    zfipg002_cab-uname       = sy-uname.
    INSERT zfipg002_cab.
    ...
    LOOP AT tprop2 WHERE hbkid <> ''.
      ...
      sy-subrc = 1.
      veces = 0.
      WHILE sy-subrc <> 0 AND veces < 121 .
...
* NEW CODE
        ...
        UP TO 1 ROWS  FROM reguv
                        WHERE laufd = tprop2-laufd
                        AND laufi   = tprop2-laufi
                        AND xvore    = 'X' ORDER BY PRIMARY KEY.
...
* END. 07-07-2026 - ATC - ATC-01
      ...
      ENDWHILE.
      zfipg002_det-bukrs       = bukrs.
      zfipg002_det-nproceso    = nproceso + 1.
      zfipg002_det-laufi       = tprop2-laufi .
      zfipg002_det-laufd       = tprop2-laufd.
      zfipg002_det-xzzmot_emis = tprop2-xzzmot_emis.
      zfipg002_det-hbkid       = tprop2-hbkid.
      zfipg002_det-zlsch       = tprop2-zlsch.
      zfipg002_det-zfbdt       = tprop2-zfbdt.
      zfipg002_det-ndocu       = tprop2-docto.
      zfipg002_det-montop      = tprop2-wrbtr.
* ini - 28-04-2022 - Waldo alarcon - Visionone.
      zfipg002_det-montop_ml    = tprop2-dmbtr.
      zfipg002_det-montop_ml_tc = tprop2-monto_ml_tc.
      zfipg002_det-waers        = tprop2-waers.
* fin - 28-04-2022 - Waldo alarcon - Visionone.
      INSERT zfipg002_det.

      IF sy-subrc <> 0 AND veces > 120.
        MESSAGE e004(zfi) WITH 'Se supero tiempo de espera(4 min).' 'Proceso detenido en propueta:' tprop2-laufd tprop2-laufi.
      ENDIF.
...
** V1 26.01.2021
    IF tprop2-zlsch = 'C'.
      ...
      REFRESH bdcdata.
*-> BEG MOD V1-CNN 12.05.2025 ECDK926256
*      PERFORM limpiar_var USING variante1.
      PERFORM clean_variant USING variante1.
*-> END MOD V1-CNN 12.05.2025 ECDK926256
      WAIT UP TO 03 SECONDS.

      REFRESH bdcdata.
*-> BEG MOD V1-CNN 12.05.2025 ECDK926256
*      PERFORM limpiar_var USING variante2.
      PERFORM clean_variant USING variante2.
*-> END MOD V1-CNN 12.05.2025 ECDK926256
      WAIT UP TO 03 SECONDS.
    ENDIF.
...
*-> BEG DEL V1-CNN 12.05.2025 ECDK926256
**&---------------------------------------------------------------------*
**&      Form  LIMPIAR_VAR
**&---------------------------------------------------------------------*
...
*
*ENDFORM.                    "LIMPIAR_VAR
*-> END DEL V1-CNN 12.05.2025 ECDK926256


*--------------------------------------------------------------------*
*    Form CLEAN_VARIANT
*--------------------------------------------------------------------*
FORM clean_variant USING iv_variant.

  DATA: ls_vari_des      TYPE varid.
  ...
  IF sy-subrc <> 0.
*   Error en determinación de variante
    MESSAGE e899(m3) WITH TEXT-e04.
  ENDIF.
  ...
  LOOP AT lt_valutab ASSIGNING FIELD-SYMBOL(<ls_valutab>).
    ...
    MOVE-CORRESPONDING <ls_valutab> TO <fs_vari_contents>.
    CASE <ls_valutab>-selname.
      WHEN 'ZW_LAUFD'.
        CLEAR: <fs_vari_contents>-low.
      WHEN 'ZW_LAUFI'.
        CLEAR: <fs_vari_contents>-low.
      WHEN 'PAR_RCHK'.
        CLEAR: <fs_vari_contents>-low.
      WHEN 'SEL_VBLN'.
        CLEAR: <fs_vari_contents>-low, <fs_vari_contents>-high.
      WHEN 'PAR_NOVO'.
        CLEAR: <fs_vari_contents>-low.
      WHEN 'PAR_NEUD'.
        CLEAR: <fs_vari_contents>-low.
  ...
  IF sy-subrc <> 0.
*   Error de actualización de variante
    MESSAGE e899(m3) WITH TEXT-e05.
  ENDIF.
