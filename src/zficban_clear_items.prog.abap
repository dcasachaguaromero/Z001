...
* NEW CODE
SELECT *
 from febko into CORRESPONDING FIELDS OF TABLE ti_febko
  where bukrs in s_bukrs
...
* NEW CODE
    SELECT *
 from febep into CORRESPONDING FIELDS OF TABLE ti_febep
      where kukey = ti_febko-kukey
            and VGINT = 'ZZ02'
            and BELNR < 1 ORDER BY PRIMARY KEY.
...
* NEW CODE
          SELECT *
 from payr into table ti_pago
            where zbukr = ti_febko-bukrs
                and HBKID = ti_febko-hbkid
                and HKTID = ti_febko-HKTID
                and CHECT = ti_febep-chect ORDER BY PRIMARY KEY.
...
* NEW CODE
SELECT *
 from t012k into CORRESPONDING FIELDS OF TABLE ti_cuentas
  where bukrs in s_bukrs
        and HBKID in s_hbkid ORDER BY PRIMARY KEY.
...
* NEW CODE
 ...
 from bsis
    into  wa_cheque
    where bukrs = ti_cuentas-bukrs
    and hkont = ti_cuentas-hkont ORDER BY PRIMARY KEY.
...
* NEW CODE
 ...
 from payr into table ti_pago
        where zbukr = ti_cuentas-bukrs
            ...
            and HKTID = ti_cuentas-HKTID
            and VBLNR = wa_cheque-belnr
            and gjahr = wa_cheque-gjahr ORDER BY PRIMARY KEY.
