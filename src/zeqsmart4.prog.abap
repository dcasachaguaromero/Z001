...
* NEW CODE
SELECT NAME_TEXT
UP TO 1 ROWS  from V_USERNAME into lv_name_text
        where BNAME = l_usr02-bname ORDER BY PRIMARY KEY.
...
* NEW CODE
      SELECT *
 FROM t000 INTO TABLE l_t000
        FOR ALL ENTRIES IN l_usr02
         WHERE mandt = l_usr02-mandt ORDER BY PRIMARY KEY.
...
* NEW CODE
    SELECT *
 FROM usvart INTO TABLE l_usvart
      FOR ALL ENTRIES IN  l_usobt_c
       WHERE varbl = l_usobt_c-low
...
* NEW CODE
 ...
 FROM usr02 CLIENT SPECIFIED
           ...
           AND  ( bname =  'SAP*'
      OR   bname = 'DDIC' OR
                         bname = 'SAPCPIC' OR  bname = 'EARLYWATCH' OR bname = 'TMSADM') ORDER BY PRIMARY KEY.
