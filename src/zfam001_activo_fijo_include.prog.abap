...
* NEW CODE
  SELECT *
 FROM anlz  INTO TABLE it_anlz WHERE BUKRS  EQ P_BUKRS
                                         AND   ANLN1  IN S_ANLN1
                                         AND   ANLN2  IN S_ANLN2
...
* NEW CODE
         SELECT *
         UP TO 1 ROWS  FROM BKPF WHERE BUKRS = IT_ANEK-AWORG+0(4)
                                   AND   AWKEY = WA_AWKEY ORDER BY PRIMARY KEY.
...
* NEW CODE
      SELECT ktansw
      UP TO 1 ROWS  INTO tabla-ktansw FROM t095
                                WHERE ktopl = p_ktopl
                                  AND ktogr = tabla-ktogr
                                  AND afabe = 1 ORDER BY PRIMARY KEY.
...
* NEW CODE
      SELECT TXK50
      UP TO 1 ROWS  INTO tabla-TXK50 FROM ANKT
                                WHERE SPRAS = sy-langu
                                  AND ANLKL = tabla-ANLKL ORDER BY PRIMARY KEY.
