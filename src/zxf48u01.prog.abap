...
* NEW CODE
SELECT valor
UP TO 1 ROWS  into lv_var
  from ZFITR020_T04
  where NOMBRE = 'FECHA_T01' ORDER BY PRIMARY KEY.
...
* NEW CODE
        SELECT *
        UP TO 1 ROWS  FROM bsik WHERE bukrs = det_bkpf-bukrs
                                  AND   lifnr = det_bseg-lifnr
                                  AND   bschl = '21'
                                  AND   zlspr = 'Z'
                                  AND   zfbdt <= det_bseg-zfbdt ORDER BY PRIMARY KEY.
...
* NEW CODE
          SELECT *
 FROM bsik     WHERE bukrs = det_bkpf-bukrs
                                  AND   lifnr = det_bseg-lifnr
                                  AND   bschl = '21'
                                  AND   zfbdt <= det_bseg-zfbdt
