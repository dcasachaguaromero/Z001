...
REPORT  Z_DI_EERR message-id 26 line-size 255
...
PARAMETER $PARAM6(10) TYPE C.

PARAMETER $PARAM7(10) TYPE C.

PARAMETER $PARAM8 TYPE D.

PARAMETER $PARAM9 TYPE D.
...
CONSTANTS C_DF_VK(40) VALUE '174'.
...
*AND ( HKONT >= $PARAM6 ) ) ) ) ).
AND ( HKONT >= $PARAM6 ) ) ) ) ) ORDER BY PRIMARY KEY .
...
ALTMP32 = BSEG-AUGBL.
 ...
 move ALTMP27 to ITAB3-USNAM.
 move ALTMP28 to ITAB3-WRBTR.
 move ALTMP29 to ITAB3-CPUDT.
 move ALTMP30 to ITAB3-CPUTM.
 move ALTMP31 to ITAB3-KOART.
 move ALTMP32 to ITAB3-AUGBL.
 move ALTMP33 to ITAB3-STBLG.
 append ITAB3.
 cntbuf = cntbuf + 1.
 if download = 'N'.
  if cntbuf > 5000.
