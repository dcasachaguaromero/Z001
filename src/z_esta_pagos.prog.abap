...
where bkpf~bukrs in p_bukrs
  ...
  and bkpf~budat in p_budat
  and bkpf~XREF2_HD in p_XREF2
  and bkpf~blart in ('B1','B2','B3','B4','B5','D1','D2','D3','D4','F0','F1','F2','F3','F4','F5','F6','F7','F8','F9','FA','FB','FC','N0','N1','N2','N3','N4','NA','NB','NC') ORDER BY PRIMARY KEY.
...
* NEW CODE
      ...
      from bsak as t1
        where
          ...
          t1~gjahr = t_bkpf-gjahr and
          t1~belnr = t_bkpf-belnr and
          t1~lifnr in p_lifnr ORDER BY PRIMARY KEY.
...
*
* NEW CODE
              SELECT lfa1~stcd1 lfa1~name1 lfa1~name2
              UP TO 1 ROWS  into (t_prov-stcd1, t_prov-name1, t_prov-name2)
              from lfa1
              where lfa1~lifnr = t_bkpf-lifnr ORDER BY PRIMARY KEY.
