   ...
   AND sy-ucomm NE 'SOC_01'
    ...
    IF  zacgl_item_tbctr-anln1  NE space.
      ...
      ELSE.
        ...
        ELSE.
          IF rf05a-buscs EQ 'G'.
            IF zacgl_item_tbctr-anbwa NE 105.
              SET CURSOR FIELD 'ZACGL_ITEM_TBCTR-ANBWA' LINE sy-stepl.
              MESSAGE e009(z1).
            ENDIF.
          ENDIF.
...
***   ti_extension2  TYPE BAPIPAREX OCCURS 1000 WITH HEADER LINE,
...
*                            t_bkpf_conta
*                  CHANGING  t_error.
* Fin Comentado por LSC

* Rutina creada por LSC 17.10.2011 - Prime Group
* Esta rutina contempla modificaciones para ajustes en monto finales y ajustes de decimales,
  PERFORM valida_doc2 TABLES t_bseg_conta
                             t_bkpf_conta
                   CHANGING  t_error.
* Fin rutina creada por LSC
...
**      INI - Q_EY_MEX  - Ajuste para marcar columna IVA - 16.01.2020
*      LOOP AT ti_detalle ASSIGNING FIELD-SYMBOL(<ls_detalle>)
*        WHERE tax_code IS NOT INITIAL.
*        <ls_detalle>-iva = abap_true.
*      ENDLOOP.
**      FIN - Q_EY_MEX  - Ajuste para marcar columna IVA - 16.01.2020

      CALL FUNCTION 'ZFIRFC001_V2'
        TABLES
...
FORM gen_det_doc  TABLES   p_t_bseg_conta STRUCTURE bseg
  ...
  LOOP AT p_t_bseg_conta.
    ...
    MOVE:   '1'  TO p_ti_detalle-key,
            ...
            p_t_bseg_conta-anbwa  TO p_ti_detalle-anbwa,
            p_t_bseg_conta-bschl  TO p_ti_detalle-bschl,
...
*      -->P_T_BKPF_CONTA  text
*----------------------------------------------------------------------*
FORM valida_doc2  TABLES   t_bseg_conta STRUCTURE bseg
                          t_bkpf_conta STRUCTURE bkpf
                CHANGING  t_error.

* Agrego variable total_detalle  20120529
* Se cambia tipo iva_prop   de konp-kbetr a  zacgl_item-wrbtr, HCD 04-01-2017
  DATA: total_pos LIKE zacgl_item-wrbtr,
        total_pos_s LIKE zacgl_item-wrbtr,
        total_pos_h LIKE zacgl_item-wrbtr,
        total_detalle LIKE zacgl_item-wrbtr,
        total_iva LIKE zacgl_item-wrbtr,
        total_iva_no_re LIKE zacgl_item-wrbtr,
        iva_prop        LIKE zacgl_item-wrbtr,
        iva            LIKE zacgl_item-wrbtr,
...
* Factura
  IF rf05a-buscs EQ 'R'.
    ...
    total_detalle = total_pos_h + total_pos_s.
...
*    ENDLOOP.
  ...
  IF total_bseg_conta EQ 0.
    ...
    t_error = 0.
  ENDIF.
** inicio si total detalle es igual a cero es porque no agregaron datos 20120529 HCD
*  IF total_detalle = 0.
*      CALL FUNCTION 'ICON_CREATE'
*      EXPORTING
*        info   = text-e12
...
*    t_error = 4.
*  ENDIF.
** fin si total detalle es igual a cero es porque no agregaron datos 20120529 HCD
ENDFORM.                    " VALIDA_DOC2
