...
*ReSQ: No Need Of Change Internal Table INT_TABLA Already Sorted
  READ TABLE int_tabla INTO zfipg002_est INDEX tabla-current_line.

* ini - 28-04-2022 - Waldo alarcon - Visionone.
  IF zfipg002_est-fecha_v IS NOT INITIAL.
    gv_waers = t001-waers.
  ENDIF.
...
*ReSQ: No Need Of Change Internal Table INT_TABLA Already Sorted
  MODIFY int_tabla FROM zfipg002_est INDEX tabla-current_line
     TRANSPORTING sel.
...
* fin - 04-06-2020 - Waldo alarcon - Visionone.
  ...
  LOOP AT tpago.
    IF tpago-shkzg = 'S'.
      ...
      MULTIPLY tpago-dmbtr       BY -1. "ini - 28-04-2022 - Waldo alarcon - Visionone.
      MULTIPLY tpago-monto_ml_tc BY -1. "ini - 28-04-2022 - Waldo alarcon - Visionone.
      MODIFY tpago FROM tpago
       TRANSPORTING wrbtr
                    dmbtr monto_ml_tc.  "ini - 28-04-2022 - Waldo alarcon - Visionone.
    ENDIF.
  ENDLOOP.
...
* ini - 28-04-2022 - Waldo alarcon - Visionone.
        tprop1-dmbtr       = tpago-dmbtr.
        tprop1-monto_ml_tc = tpago-monto_ml_tc.
        tprop1-waers       = tpago-waers.
* fin - 28-04-2022 - Waldo alarcon - Visionone.
        APPEND tprop1.
      ENDLOOP.

* ini - 04-06-2020 - Waldo alarcon - Visionone.
      IF int_tabla-bvtyp_existe EQ 'X'.
        MOVE TEXT-e03 TO gv_mensaje.
      ENDIF.
* fin - 04-06-2020 - Waldo alarcon - Visionone.
  ...
  SORT tprop1 BY zfbdt  hbkid zlsch zzmot_emis.

  LOOP AT tprop1.
    MOVE-CORRESPONDING tprop1 TO wa_tprop1.
...
* ini - 28-04-2022 - Waldo alarcon - Visionone.
      tprop2-dmbtr       = tprop1-dmbtr.
      tprop2-monto_ml_tc = tprop1-monto_ml_tc.
      tprop2-waers       = wa_tprop1-waers.
* fin - 28-04-2022 - Waldo alarcon - Visionone.
      APPEND tprop2.
      CLEAR tprop2.
    ENDAT.
  ...
  SELECT MAX( laufi ) INTO clave2   FROM zfipg002_det WHERE laufd = fecha0
                                    AND  laufi LIKE clave.
...
* ini - 28-04-2022 - Waldo alarcon - Visionone.
    int_tabla2-monto_ml    = tprop2-dmbtr.
    int_tabla2-monto_ml_tc = tprop2-monto_ml_tc.
    int_tabla2-waers        = tprop2-waers.
* fin - 28-04-2022 - Waldo alarcon - Visionone.
    APPEND int_tabla2.
  ENDLOOP.
  ...
  tabla2-lines    = fill.
  tabla2-top_line = 1.
  CLEAR zfipg002_cab-descr.

  CALL SCREEN 250 STARTING AT 20 05 ENDING AT 154 19.
...
***                    AND   wrbtr = int_tabla-monto.
      ...
      tabla3-lines = fill.
      tabla3-top_line = 1.
      LOOP AT tabla3-cols INTO cols." where index = 14 .  " elimina MSJ
        cols-index        = sy-tabix.
        IF cols-screen-name EQ 'ZFIPG002_B_EST-MSG'.
          cols-screen-input = 0.
          cols-invisible    = '1'.
        ENDIF.
        MODIFY tabla3-cols FROM cols INDEX sy-tabix.
      ENDLOOP.
      SORT int_tabla3 BY zzmot_emis gjahr  belnr buzei.
      CALL SCREEN 350 STARTING AT 10 05 ENDING AT 135 24.
    WHEN 'ZFIPG002_EST-MONTO_R'.
...
***                    AND   wrbtr = int_tabla-monto.
        ...
        int_tabla3-dmbtr = tpago-dmbtr_r. "ini 28-04-2022 - Waldo alarcon - Visionone.
        APPEND int_tabla3.
      ENDLOOP.
      DESCRIBE TABLE int_tabla3 LINES fill.
      tabla3-lines = fill.
    ...
    WHEN 'ZFIPG002_EST-MONTO_NA'.
      ...
      LOOP AT tpago WHERE msg <> ''
        ...
        int_tabla3-dmbtr = tpago-dmbtr_na. "ini 28-04-2022 - Waldo alarcon - Visionone.
        APPEND int_tabla3.
      ENDLOOP.
      DESCRIBE TABLE int_tabla3 LINES fill.
      tabla3-lines = fill.
...
* NEW CODE
  ...
  ENDSELECT.
* END. 07-07-2026 - ATC - ATC-01"#EC CI_SGLSELECT
**add comment fin
* BEGIN. 07-07-2026 - ATC - ATC-01
* OLD CODE
...
ENDFORM.                    " BI_CARTA
*&---------------------------------------------------------------------*
*&      Module  MARCA_COLUMNA  OUTPUT
*&---------------------------------------------------------------------*
*       text
*----------------------------------------------------------------------*
MODULE marca_columna OUTPUT.
*tabla-current_line.
  IF zfipg002_est-bvtyp_existe EQ 'X' AND zfipg002_est-monto NE 0.
    LOOP AT SCREEN.
      CHECK screen-name EQ 'ZFIPG002_EST-MONTO' OR
            screen-name EQ 'ZFIPG002_EST-DESCR'.
