...
* NEW CODE
  SELECT *
  UP TO 1 ROWS 
        FROM ztd_pagobanco  WHERE banco = tabla_00-ubnkl ORDER BY PRIMARY KEY.
...
* NEW CODE
 ...
 INTO CORRESPONDING FIELDS OF TABLE  tiposdoc
          FROM ztd_pagobanco WHERE banco = bancoref ORDER BY PRIMARY KEY.
...
* NEW CODE
  SELECT adrnr
  UP TO 1 ROWS  INTO v_adrnr
*V1 RVY 24.09.2020
*        FROM t001           WHERE bukrs EQ tabla_00-zbukr.
*V1 RVY 24.09.2020
         FROM t001           WHERE bukrs EQ tabla_00-absbu ORDER BY PRIMARY KEY.
...
*                  AND   augbl EQ regup-vblnr
*V1 RVY 24.09.2020
                   AND   belnr EQ regup-belnr ORDER BY PRIMARY KEY.
...
* NEW CODE
 SELECT *
 UP TO 1 ROWS  INTO wa_zfitr005
    FROM zfitr005 WHERE bukrs  EQ tabla_00-zbukr
                    AND hbkid  EQ tabla_00-hbkid
                    AND zmotiv EQ bsak-zzmot_emis ORDER BY PRIMARY KEY.
...
* NEW CODE
    SELECT adrnd
    UP TO 1 ROWS      INTO v_adrnr
           FROM knvk  WHERE lifnr EQ tabla_00-lifnr ORDER BY PRIMARY KEY.
...
* NEW CODE
    SELECT smtp_addr
    UP TO 1 ROWS  INTO v_mail
           FROM adr6  WHERE addrnumber EQ tabla_00-adrnr ORDER BY PRIMARY KEY.
