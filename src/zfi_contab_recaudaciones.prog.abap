...
* NEW CODE
      SELECT bukrs
        ...
        WHERE bukrs = ls_messtab-msgv2
          AND belnr = ls_messtab-msgv1
          AND gjahr = wa_data-fecha_contable+4(4)
          AND kunnr = l_kunnr_dpp ORDER BY PRIMARY KEY.
...
* NEW CODE
        ...
        FOR ALL ENTRIES IN t_bsid
          WHERE bukrs = t_bsid-bukrs
           AND belnr = t_bsid-belnr
           AND gjahr = t_bsid-gjahr
           AND buzei = t_bsid-buzei ORDER BY PRIMARY KEY.
...
* NEW CODE
  ...
  UP TO 1 ROWS 
    INTO CORRESPONDING FIELDS OF wa_fpagrec
    FROM zfifpagrec
    WHERE fpago = wa_data-forma_pago  ORDER BY PRIMARY KEY.
...
* NEW CODE
    ...
    UP TO 1 ROWS 
     FROM zfimtvrechz
     INTO wa_zfimtvrechz
     WHERE zmtvrechz = wa_data-rechazo ORDER BY PRIMARY KEY.
...
* NEW CODE
      SELECT bukrs kunnr belnr rebzg wrbtr
        ...
        WHERE bukrs = wa_bsid-bukrs
          AND kunnr = wa_bsid-kunnr
          AND rebzg = wa_bsid-belnr ORDER BY PRIMARY KEY.
...
* NEW CODE
  ...
  WHERE bukrs = wa_data-sociedad AND
      kunnr = l_kunnr AND
      bschl = '01' AND
      zuonr  = wa_data-evento ORDER BY PRIMARY KEY.
...
*          ( bschl =  '01'  OR  bschl = '15' ) AND
          bschl = '01' AND
          vertn = l_vertn AND
          vertt = wa_data-clase_contrato ORDER BY PRIMARY KEY.
...
* NEW CODE
  SELECT bukrs
    ...
    WHERE bukrs = wa_data-sociedad         AND
          ...
          vertn = l_vertn                  AND
          vertt = wa_data-clase_contrato   AND
          xref1 = wa_data-cheque ORDER BY PRIMARY KEY.
...
* NEW CODE
  ...
  UP TO 1 ROWS 
    ...
    WHERE bukrs = wa_data-sociedad AND
          kunnr = l_kunnr AND
          vertn = l_vertn ORDER BY PRIMARY KEY. 

  ENDSELECT.
* END. 07-07-2026 - ATC - ATC-01" wa_data-contrato.
...
* NEW CODE
    SELECT kunnr
      ...
      INTO p_kunnr
      FROM kna1 UP TO 1 ROWS
       WHERE stcd1 = l_stcd1 ORDER BY PRIMARY KEY.
