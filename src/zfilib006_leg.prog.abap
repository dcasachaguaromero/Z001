...
REPORT  zfilib006_leg  MESSAGE-ID    icc_cl
...
****               AND    augdt       IN s_augdt
               AND    blart       IN s_blart
               AND    belnr       IN s_belnr ORDER BY PRIMARY KEY.
...
* NEW CODE
 ...
 FROM  with_item  INTO CORRESPONDING FIELDS
           ...
           AND    witht         IN qsskzi
           AND    wt_acco     = part_work-lifnr
           AND    wt_withcd     IN s_codei ORDER BY PRIMARY KEY.
...
* NEW CODE
 ...
 FROM t059p WHERE land1 = country
                     AND   witht IN qsskzi ORDER BY PRIMARY KEY.
...
* NEW CODE
    SELECT *
    UP TO 1 ROWS  FROM tgsb
           WHERE gsber = s_busa ORDER BY PRIMARY KEY.
...
* NEW CODE
      SELECT *
      UP TO 1 ROWS  FROM tgsbt WHERE  spras = t001-spras
                                  AND    gsber = tgsb-gsber ORDER BY PRIMARY KEY.
...
*          text-405, "num doc
          ...
          30 text-402, "Nombre.
          70 text-404, "Boleta
          90 text-398, "Fecha de referencia
          110 text-406, "Base
          135 text-407, "Impuesto
          160 text-408. "Total
  ULINE.
  SKIP.
