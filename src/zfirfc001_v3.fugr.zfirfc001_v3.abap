...
*&---------------------------------------------------------------------*
FUNCTION zfirfc001_v3.
*"----------------------------------------------------------------------
*"*"Interfase local
*"  IMPORTING
*"     VALUE(I_CARGA_MASIVA) TYPE  XFLAG OPTIONAL
*"  TABLES
*"      TI_CABECERA STRUCTURE  ZCABECERAV3
*"      TI_DETALLE STRUCTURE  ZDETALLEV3
*"      TI_TLOGCABERR STRUCTURE  ZTLOGCABERRV3
*"      TI_TLOGDETERR STRUCTURE  ZTLOGDETERRV3
*"      TI_RESUMEN STRUCTURE  ZRESUMENV3
...
*  Limpia Tablas Internas.
  REFRESH: return, return2, currencyamount, accountpayable, accountreceivable, accountgl, documentheader, currencyamount, extension1,
           accountwt, accounttax, t_mwdat, ti_cont_det, ti_cont_cab, ti_error_det, ti_error_cab, ti_resumen.
...
* Verifico  si docuemnetos enviados esta en proceso.
* ini Waldo Alarcón - Visionone - 11-11-2024
  g_carga_masiva = i_carga_masiva.
* fin Waldo Alarcón - Visionone - 11-11-2024

  LOOP AT  ti_cabecera.
    IF ti_cabecera-recarga  = 'V'.
* BEGIN. 07-07-2026 - ATC - ATC-01
* OLD CODE
*      SELECT SINGLE * FROM zfirfc04 WHERE   bukrs = ti_cabecera-comp_code
*                                    AND     grupo = ti_cabecera-grupo
*                                    AND     zkey  =  ti_cabecera-key.
*
* NEW CODE
      SELECT *
      UP TO 1 ROWS  FROM zfirfc04 WHERE   bukrs = ti_cabecera-comp_code
                                    AND     grupo = ti_cabecera-grupo
...
* BEGIN. 07-07-2026 - ATC - ATC-01
* OLD CODE
*      SELECT SINGLE * FROM zfirfc03 WHERE  bukrs = ti_cabecera-comp_code
*                                       AND    grupo = ti_cabecera-grupo.
*
* NEW CODE
      SELECT *
      UP TO 1 ROWS  FROM zfirfc03 WHERE  bukrs = ti_cabecera-comp_code
...
* BEGIN. 07-07-2026 - ATC - ATC-01
* OLD CODE
*        SELECT SINGLE * FROM zfirfc02 WHERE  bukrs = ti_cabecera-comp_code
*                                      AND    grupo = ti_cabecera-grupo
*                                      AND    lineas = lineas
*                                      AND    valord = valord
*                                      AND    valorh = valorh.
...
* NEW CODE
        SELECT *
        UP TO 1 ROWS  FROM zfirfc02 WHERE  bukrs = ti_cabecera-comp_code
                                      AND    grupo = ti_cabecera-grupo
                                      AND    lineas = lineas
                                      AND    valord = valord
...
* END. 07-07-2026 - ATC - ATC-01
        ...
        IF sy-subrc = 0.
          SELECT MAX( secuencia ) INTO gsecuencia  FROM zfirfc02 WHERE  bukrs = ti_cabecera-comp_code
                                    AND    grupo = ti_cabecera-grupo
                                    AND    lineas = lineas
                                    AND    valord = valord
                                    AND    valorh = valorh.
...
* END. 07-07-2026 - ATC - ATC-01
          ...
          IF ti_cabecera-recarga <> 'R'.
            ...
            return-message_v4        = ti_cabecera-key.
            APPEND return.
            SELECT MAX( secuencia ) INTO xsecuencia  FROM zfirfc01
...
* OLD CODE
*            SELECT  *  FROM zfirfc01 WHERE bukrs     =  ti_cabecera-comp_code
*                                           AND   grupo     =  ti_cabecera-grupo
*                                           AND   zkey      =  zfirfc02-zkey
...
* NEW CODE
            SELECT *
  FROM zfirfc01 WHERE bukrs     =  ti_cabecera-comp_code
                                           AND   grupo     =  ti_cabecera-grupo
                                           AND   zkey      =  zfirfc02-zkey
...
*  Contabilización de Documentos.
      ...
      LOOP AT ti_cont_cab.
        ...
        CLEAR zfirfc01.
        SELECT MAX( secuencia ) INTO (secuencia)  FROM zfirfc01 WHERE bukrs  =  ti_cont_cab-comp_code
