...
**        numck(13)     TYPE c,    "10 numero cheque
...
****************Definición de Metodos*******************************************
    METHODS: m_get_data      IMPORTING value(wi_file)      TYPE filename
             ...
             m_procesa_bci_XLS IMPORTING value(wi_bnco)    TYPE ty_banco
                               EXPORTING value(ito_c)      TYPE ito_c
                                         value(ito_d)      TYPE ito_d,
             ...
             m_procesa_bbva_NEW   IMPORTING value(wi_bnco) TYPE ty_banco
                             EXPORTING value(ito_c)        TYPE ito_c
                                       value(ito_d)        TYPE ito_d,
             ...
             m_procesa_corp_xls   IMPORTING value(wi_bnco) TYPE ty_banco
                             EXPORTING value(ito_c)        TYPE ito_c
                                       value(ito_d)        TYPE ito_d,
** V1 RVY 20-04-2021
             m_procesa_itau_xls   IMPORTING value(wi_bnco) TYPE ty_banco
                             EXPORTING value(ito_c)        TYPE ito_c
                                       value(ito_d)        TYPE ito_d,
** V1 RVY 20-04-2021
             ...
             m_procesa_estado_ANT  IMPORTING value(wi_bnco) TYPE ty_banco
                             EXPORTING value(ito_c)        TYPE ito_c
                                       value(ito_d)        TYPE ito_d,

             m_procesa_estado_NEW  IMPORTING value(wi_bnco) TYPE ty_banco
                             EXPORTING value(ito_c)        TYPE ito_c
                                       value(ito_d)        TYPE ito_d,
...
*               EXPORTING value(ito_c)        TYPE ito_c
*                         value(ito_d)        TYPE ito_d,
            m_procesa_bice_txt       IMPORTING value(wi_bnco) TYPE ty_banco
                             EXPORTING value(ito_c)        TYPE ito_c
                                       value(ito_d)        TYPE ito_d,
             ...
             m_dwn_file      IMPORTING value(iti_c)        TYPE ito_c     "#EC "#EC CI_VALPAR
                                       value(iti_d)        TYPE ito_d     "#EC "#EC CI_VALPAR
                             EXPORTING value(w_flag1)      TYPE c,        "#EC "#EC CI_VALPAR

             m_find_file     EXPORTING value(p_file1)      TYPE filename,

             m_valida_bco    EXPORTING value(wi_bnco)      TYPE ty_banco. "#EC "#EC CI_VALPAR
...
*SORT IT_DATA ."JOROZCO 20.01.2020
...
* LEEMOS EL DETALLE REGISTRO 2
      IF wa_data+0(1) = '2'.  "Registro de detalle
        ...
        ELSE.
** V1 RVY 26.01.202
**          wa_det-numck    = '0000000000000'.
          wa_det-numck      = '00000000'.
**         wa_det-numck = wa_data+32(8). "Número de Cheque o deposito u otro tipo de documento.
**         wa_det-numck+5(8)    = wa_data+32(8). "Número de Cheque o deposito u otro tipo de documento.
          wa_det-numck(8)   = wa_data+32(8). "Número de Cheque o deposito u otro tipo de documento.
**         wa_det-not_pay01     = wa_data+32(8). "Número de Cheque o deposito u otro tipo de documento
          wa_det-not_pay01  = wa_det-numck.
** V1 RVY 26.01.2021
        ENDIF.
...
*Recorremos y grabamos en las lineas de cabecera y detalle
  ...
  ENDMETHOD.                    "M_PROCESA_BCI

** V1 INI RVY 04.08.23
*---------------------------
* PROCESA BANCO BCI XLS
*---------------------------
 METHOD m_procesa_bci_XLS.

 TYPES: BEGIN OF ty_bci,
          fecha(100)   TYPE c,
          colb(100)    TYPE c,
          sucur(100)   TYPE c,
          cold(100)    TYPE c,
    ...
    DATA:  it_bci        TYPE STANDARD TABLE OF ty_bci,
           wa_bci        TYPE ty_bci,
           it_cab        TYPE STANDARD TABLE OF ty_mcc,
           wa_cab        TYPE ty_mcc,
           it_det        TYPE STANDARD TABLE OF ty_mcd,
...
*
    ...
    LOOP AT it_bci INTO wa_bci WHERE fecha NS '-'.
      IF wa_bci-fecha = 'N° DE CUENTA'.
        ctacte_n = wa_bci-cole.
      ENDIF.
    ENDLOOP.
    ...
    LOOP AT it_bci INTO wa_bci WHERE fecha CS 'al'.
      CHECK wa_bci-fecha IS NOT INITIAL.
      wa_det-cod_bank = wa_cab-cod_bank.
      wa_det-ctbkn    = wa_cab-ctbkn.
      CONCATENATE wa_bci-fecha+14(2) '.' wa_bci-fecha+17(2) '.'
                  wa_bci-fecha+22(2) INTO wa_det-fec_val.
      wa_cab-FEC_CONT  = wa_det-fec_val.
      wa_det-num_ctla  = wa_cab-ctbkn.
      wa_det-p_date    = wa_det-fec_val.
      wa_det-prnot_num = wa_cab-moneda.
      CONCATENATE wa_bci-fecha+17(2) wa_bci-fecha+14(2) INTO wa_cab-num_ctla.
      CLEAR aux_monto.
      aux_monto      = wa_bci-cole.
      wa_cab-sal_ini = aux_monto.

      aux_monto      = wa_bci-colh.
      wa_cab-tot_c   = aux_monto.

      aux_monto      = wa_bci-colj.
      wa_cab-tot_a   = aux_monto.

      aux_monto      = wa_bci-abono.
      wa_cab-sal_fin = aux_monto.
    ENDLOOP.
    ...
    LOOP AT it_bci INTO wa_bci WHERE fecha CS '/'.
      CHECK wa_bci-fecha IS NOT INITIAL.
      nrodoc           = wa_bci-docto.
      wa_det-num_ctla  = wa_cab-num_ctla.
      wa_det-not_pay01 = nrodoc.
      wa_det-numck     = wa_det-not_pay01.
      wa_cab-num_ctla  = wa_cab-num_ctla.
      w_desc           = wa_bci-desc+0(10).
      ...
      IF wa_det-bank_postext = '0000'.
         IF signo = '-'.
            wa_det-bank_postext = '0A'.
         else.
            wa_det-bank_postext = '2V'.
         endif.
      endif.
      ...
      wa_det-not_pay03 = wa_bci-desc+0(27).
      wa_det-not_pay04 = wa_bci-desc+27(27).
    ...
    LOOP AT it_data INTO wa_data.
      ...
      IF wa_secd-tipo = '1'.
        ...
        IF sy-subrc EQ 0.
          ...
          n_sum = n_sum - 2.
        ENDIF.
        wa_secc-fec_cont = wa_data+n_sum(10).
        n_sum = n_sum - n_ctla.
        wa_secc-num_ctla = wa_data+n_ctla(n_sum).
        wa_secc-moneda = 'CLP'.
*          FIND '+'
*            IN WA_DATA
*             MATCH OFFSET moff
...
*          SAL_FIN = SAL_FIN - AUX_MONTO.
        ...
        ELSE.
*Begin of change: ReSQ Correction INDEX READ on an unsorted Internal TABLE 19/12/2019 EY_DES04 ECDK917080 *
*SORT RESULT_TAB ."JOROZCO 20.01.2020
*End of change: ReSQ Correction for INDEX READ on an unsorted Internal TABLE 19/12/2019 EY_DES04 ECDK917080 *
          tot_a = tot_a + aux_monto.
*          SAL_FIN = SAL_FIN + AUX_MONTO.
          CONCATENATE '+' aux_monto INTO wa_secd-monto.
...
*SORT IT_BBVA . "JOROZCO 20.01.2020
...
*           wa_cabec-ctbkn = desc.
            CONCATENATE '010' desc+3(7) INTO wa_cabec-ctbkn.
          ENDIF.
...
*** V1 RVY 27.09.2021 y 13-10-2021
       IF wa_corp-mto_cgo IS NOT INITIAL.
***    IF wa_corp-mto_cgo <> '$0'.
          signo = '-'.
...
*ReSQ: No Need Of Change Internal Table RESULT_TAB Already Sorted
  ...
  ENDMETHOD.                    "M_PROCESA_ITAU_XLS

*---------------------------
* PROCESA BANCO ESTADO Formto Nuevo
*---------------------------
...
*Begin of change: ReSQ Correction for DELETE on an unsorted Internal Table 19/12/2019 EY_DES04 ECDK917080 *
*SORT IT_EST . "JOROZCO 20.01.2020
*End of change: ReSQ Correction for DELETE on an unsorted Internal Table 19/12/2019 EY_DES04 ECDK917080 *
    CLEAR nom_mov.
...
*      IF wa_est-mesdia EQ 'CUENTA:'.
*        ctacte = wa_est-dato.
      IF nom_mov = '5'.
         ctacte = wa_est-sucur.
      ENDIF.

*     IF wa_est-mesdia CS 'FECHA EMISI'.
*        fechamov =  wa_est-dato.
      IF wa_est-sucur CS 'Fecha de  '.
         fechamov =  wa_est-desc.
      ENDIF.

*      IF wa_est-mesdia CS 'Nº CARTOLA'.
      IF wa_est-sucur CS 'N° Cartola'.
*       num_ctla_n =  wa_est-dato.
        num_ctla_n =  wa_est-desc.
        num_ctla_i = num_ctla_n.
      ENDIF.

*     IF wa_est-mesdia CS 'ANTERIOR'.
*       saldoini = wa_est-dato.
     IF wa_est-sucur CS 'Saldo Inicial'.
        saldoini = wa_est-desc.
...
*      IF wa_est-mesdia CS 'FINAL'.
*        saldofin = wa_est-dato.
      IF wa_est-sucur CS 'Saldo Final'.
        saldofin = wa_est-desc.
...
*    --------------------------
    LOOP AT it_est INTO wa_est.
      ...
      wa_det-ctbkn = wa_cab-ctbkn.
      wa_det-num_ctla = wa_cab-num_ctla.
*      CONCATENATE wa_est-mesdia+0(2) '.' wa_est-mesdia+3(2) '.' wa_est-mesdia+8(2) INTO wa_det-fec_val.
      CONCATENATE wa_est-sucur+0(2) '.' wa_est-sucur+3(2) '.' wa_est-sucur+8(2) INTO wa_det-fec_val.
      wa_det-prnot_num = wa_cab-moneda.
*      nrodoc = wa_est-nro_ope.
      nrodoc = wa_est-cargo.
      wa_det-not_pay01 = nrodoc.
      wa_det-numck = wa_det-not_pay01.
*      IF wa_est-cargo NE ' ' and wa_est-cargo NE '0'.
*         aux_n = wa_est-cargo.
*         signo = '-'.
*         tot_c = tot_c + aux_n.
*      ELSE.
...
*         tot_a = tot_a + aux_n.
*      ENDIF.
      IF wa_est-cta_cte+1(1) NE ' ' and wa_est-cta_cte+1(1) NE '0'.
         aux_n = wa_est-cta_cte.
         signo = '-'.
         tot_c = tot_c + aux_n.
      ELSE.
         aux_n = wa_est-fec_movto.
         signo = '+'.
         tot_a = tot_a + aux_n.
      ENDIF.
...
*---------------------------
  METHOD m_procesa_estado_new.
    ...
    DATA: ctacte(20)   TYPE c,
      ...
      zzrefsuc TYPE zrefsuc,
*V1 RVY 18-10-2024
      wa_ctla_ant(4) TYPE    c ,
      ctacte_n(6) TYPE n,
      nrodoc(8) TYPE n,
      tot_c(18) TYPE n,
      tot_a(18) TYPE n,
      wa_cab_ctbkn2(24) type c,
      it_est TYPE STANDARD TABLE OF ty_est,
      wa_est TYPE ty_est,
      it_cab TYPE STANDARD TABLE OF ty_mcc,
      wa_cab TYPE ty_mcc,
...
*
    ...
    LOOP AT it_est INTO wa_est.
      ADD 1 TO nom_mov.
      IF nom_mov = 1.
* primer regisrro de la cartola
         saldoini        = wa_est-sal_ini.
         num_ctla        = wa_est-cartola.
         saldofin        = wa_est-sal_fin.
         wa_cab-cod_bank = '012'.
         wa_cab-ctbkn    = wa_est-cta_cte.
         wa_cab_ctbkn2   = wa_est-cta_cte.
         largo  = STRLEN( wa_est-cta_cte ).
         largo2 = largo - 6.
         if largo2 > 0.
            wa_cab-ctbkn = wa_est-cta_cte+largo2(6).
         endif.
         IF wa_est-fecha+6(2) = '23'.
            num_ctla2 = num_ctla + 1000.
         else.
            num_ctla2 = num_ctla.
         endif.
         wa_cab-fec_cont = wa_det-fec_val.
         CONCATENATE wa_est-mesdia+2(2) wa_est-mesdia+0(2) INTO num_ctla2.
         wa_cab-num_ctla = num_ctla2.
         wa_ctla_ant     = num_ctla2.

         CONCATENATE wa_est-mesdia+0(2) '.' wa_est-mesdia+2(2) '.'
                     wa_est-fecha+6(2)
                     INTO wa_det-fec_val.
         wa_cab-fec_cont = wa_det-fec_val.
         wa_cab-moneda   = 'CLP'.
         wa_cab-sal_ini  = saldoini.
         wa_cab-sal_fin  = saldofin.
         wa_cab-in_cta   = wa_est-cta_cte.
         IF wa_est-car_abo = 'C'.
            wa_cab-tot_c = wa_est-monto.
            tot_c        = wa_est-monto.
            wa_cab-tot_a = 0.
            tot_a        = 0.
            signo        = '-'.
         else.
            wa_cab-tot_a = wa_est-monto.
            tot_a        = wa_est-monto.
            wa_cab-tot_c = 0.
            tot_c        = 0.
            signo        = '+'.
...
** Detalle
         ...
         wa_det-ctbkn    = wa_cab-ctbkn.
         wa_det-num_ctla = wa_cab-num_ctla.
*         CONCATENATE wa_est-fecha+0(2) '.' wa_est-fecha+2(2) '.'
*                     wa_est-fecha+6(2)
*                     INTO wa_det-fec_val.
*         wa_cab-fec_cont  = wa_det-fec_val.
         wa_det-prnot_num = wa_cab-moneda.
         wa_det-not_pay01 = wa_est-numdoc.
         wa_det-numck     = wa_det-not_pay01.
         CONCATENATE signo wa_est-monto INTO wa_det-monto.
         wa_det-bank_postext = wa_est-cod_ope.
         wa_det-p_date       = wa_det-fec_val.
*        wa_det-not_pay02 =
         wa_det-not_pay03 = wa_est-desc+0(27).
         wa_det-not_pay04 = wa_est-desc+27(27).
...
*V1 RVY 18-10-2024
         wa_cab-fec_cont = wa_det-fec_val.
         CONCATENATE wa_est-mesdia+2(2) wa_est-mesdia+0(2) INTO num_ctla2.
         IF wa_cab_ctbkn2 = wa_est-cta_cte.
            IF wa_ctla_ant = num_ctla2.
*     se repite la isma cartola
               saldofin = wa_est-sal_fin.
               IF wa_est-car_abo = 'C'.
                  tot_c = tot_c + wa_est-monto.
                  signo = '-'.
               else.
                  tot_a = tot_a + wa_est-monto.
                  signo = '+'.
               endif.
               wa_cab-tot_c = tot_c.
            else.
               wa_cab-tot_a = tot_a.
               wa_cab-sal_fin = wa_cab-sal_ini + tot_a - tot_c.
               APPEND wa_cab TO it_cab.
               wa_ctla_ant     = num_ctla2.
               saldoini        = wa_est-sal_ini.
               num_ctla        = wa_est-cartola.
               ...
               largo = STRLEN( wa_est-cta_cte ).
               if largo > 6.
                  wa_cab-ctbkn = wa_est-cta_cte+1(6).
               endif.
               IF wa_est-fecha+6(2) = '23'.
                  num_ctla2 = num_ctla + 1000.
               else.
...
*V1 RVY 21-10-2024
*      move wa_cab-num_ctla to wa_det-num_ctla.
*      move wa_cab-fec_cont to wa_det-fec_val.
*      move wa_cab-fec_cont to wa_det-p_date.
      CONCATENATE
                    wa_det-cod_bank
                    wa_det-ctbkn
                    wa_det-num_ctla
      ...
      APPEND wa_d TO ito_d.
    ENDLOOP.
  ENDMETHOD.                    "M_PROCESA_ESTADO_NEW

*---------------------------
* PROCESA BANCO ESTADO formato antiguo
*---------------------------
  METHOD m_procesa_estado_ANT.
    TYPES:     BEGIN OF ty_est,
        mesdia(100) TYPE c,
        dato(100) TYPE c,
        sucur(100) TYPE c,
...
*EXIT.
    ...
    LOOP AT it_det INTO wa_det.
      ...
      APPEND wa_d TO ito_d.
    ENDLOOP.
  ENDMETHOD.                    "M_PROCESA_ESTADO ANTIGUO
...
*  ENDMETHOD.                    "M_PROCESA_BICE_HTM
  ...
  DATA: up_date6      LIKE sy-datum,   " Variable Fecha
        up_varnum(18) TYPE n,
        up_varnum2(7) TYPE n,
        sum_cargo(14) TYPE p DECIMALS 0,
        sum_abono(14) TYPE p DECIMALS 0,
        aux_saldo1(15) TYPE p DECIMALS 0,
        aux_saldo2(15) TYPE p DECIMALS 0,
        up_salfin(14) TYPE p DECIMALS 0,
        up_totpar(14) TYPE p DECIMALS 0,
        up_salant(14) TYPE p DECIMALS 0,
        up_saldo(14)  TYPE p DECIMALS 0,
        up_cargo(14)  TYPE p DECIMALS 0,
        up_abono(14)  TYPE p DECIMALS 0.
  ...
  DATA: ws_flag       TYPE c VALUE '0',
        ws_carg       TYPE c VALUE '0'.

  DATA: BEGIN OF cab_bice,       "Formato BICE
        tip_reg(01)    TYPE c,
        fil_001(06)    TYPE c,
        cod_bco(03)    TYPE c,         "Codigo de banco
        fil_002(03)    TYPE c,
        num_cta(08)    TYPE c,         "Número de Cuenta
        fil_003(46)    TYPE c,
        sal_fin(13)    TYPE n,         "Saldo Cartola
        sig_sal(01)    TYPE c,
        fil_004(03)    TYPE c.
  DATA: END OF cab_bice.

  DATA: BEGIN OF det_bice,       "Formato BICE
        tip_reg(01)    TYPE c,
        fil_001(06)    TYPE c,
        BEGIN OF fec_mov,
              ano_mov(4) TYPE c,
        ...
        END OF fec_mov,
        fil_002(01)    TYPE c,
        cod_mov(02)    TYPE n,         "Codigo del Movimiento
        glo_mov(30)    TYPE c,         "Glosa del Movimiento
        num_doc(07)    TYPE c,         "Número de Documento
        mon_mov(13)    TYPE n,         "Monto del Movimiento
        sig_mov(01)    TYPE c,         "Signo del Monto
        fil_004(15)    TYPE c,
  END OF det_bice.

  DATA: BEGIN OF tot_bice,       "Formato BICE
        tip_reg(01)    TYPE c,
        fil_000(83)    TYPE c,
  END OF tot_bice.
*
...
*  REFRESH: it_cab, it_det.
  LOOP AT it_data INTO wa_data.
*   PERFORM limp_detalle.
    IF wa_data+0(1) = '1'.
      MOVE wa_data          TO cab_bice.
      IF cab_bice-sig_sal = '-'.
         up_salfin = cab_bice-sal_fin * -1.
         up_saldo = up_salant.
      ELSE.
         up_salfin = cab_bice-sal_fin.
         up_saldo = up_salant.
      ENDIF.

    ELSE.
      IF wa_data+0(1) = '2'.
        MOVE wa_data      TO det_bice.
        IF det_bice-cod_mov = '00'.
          IF det_bice-sig_mov = '-'.
            up_salant = det_bice-mon_mov * -1.
            up_saldo = up_salant.
          ELSE.
            up_salant = det_bice-mon_mov.
            up_saldo = up_salant.
          ENDIF.
        ELSE.
          CONCATENATE det_bice-fec_mov+6(2)
                      det_bice-fec_mov+4(2)
                      det_bice-fec_mov+2(2)
               INTO fec_pas SEPARATED BY '.'.
          mon_pas = det_bice-mon_mov.
          up_cargo  = 0.
          up_abono  = 0.
          IF ws_flag = '0'.
            ws_flag = '1'.
            up_totpar = det_bice-mon_mov.
            IF det_bice-sig_mov = '+'.
              MULTIPLY up_totpar BY -1.
            ENDIF.
            wa_cab-cod_bank   = cab_bice-cod_bco.
            wa_cab-ctbkn      = cab_bice-num_cta.
            CONCATENATE det_bice-fec_mov+4(2)
                        det_bice-fec_mov+6(2)
               INTO wa_cab-num_ctla.
            wa_cab-moneda     = 'CLP'.
            wa_cab-fec_cont   = fec_pas.
            up_varnum         = up_salant.
            wa_cab-sal_ini    = up_varnum.
            IF up_salant LE 0.
              wa_cab-sal_ini+0(1) = '-'.
            ENDIF.
          ENDIF.
          IF det_bice-sig_mov = '-'.
            up_cargo           = det_bice-mon_mov.
            wa_det-monto+4(14) = det_bice-mon_mov.
            wa_det-monto+0(1)  = '-'.
            ADD up_cargo TO sum_cargo.
            SUBTRACT up_cargo FROM up_saldo.
          ELSE.
            up_abono           = det_bice-mon_mov.
            wa_det-monto+4(14) = det_bice-mon_mov.
            wa_det-monto+0(1)  = '+'.
            ADD up_abono TO sum_abono.
            ADD up_abono TO up_saldo.
          ENDIF.
          wa_det-cod_bank = wa_cab-cod_bank.
          wa_det-ctbkn    = wa_cab-ctbkn.
          wa_det-num_ctla = wa_cab-num_ctla.
          wa_det-fec_val  = fec_pas.
          wa_det-prnot_num = wa_cab-moneda.
          wa_det-p_date    = wa_det-fec_val.
          wa_det-cod_op    = det_bice-cod_mov.
          wa_det-not_pay03 = det_bice-glo_mov.
*          CALL FUNCTION 'ZCOD_OPERACION'
*             EXPORTING
*               i_cbanco = wi_bnco
*               i_cbusq  = 'C'
*               i_movbco = signo
*               i_desc   = wa_det-cod_op
*             IMPORTING
*               e_codigo = tbankl.
          wa_det-bank_postext = tbankl.
          wa_det-not_pay02 = '000'.
          wa_det-not_pay03 = det_bice-glo_mov+0(27).
          wa_det-not_pay04 = det_bice-glo_mov+27(03).
*
          IF det_bice-num_doc NE '0000000'.
            up_varnum2   = det_bice-num_doc.
            wa_det-numck = up_varnum2.
          ELSE.
            CLEAR wa_det-numck.
          ENDIF.
    ...
    ENDIF.
    IF wa_data+0(1) = '3'.
       MOVE wa_data TO tot_bice.
    ENDIF.
  ENDLOOP.

  aux_saldo1 = up_salant.
  ADD sum_abono TO aux_saldo1.
  SUBTRACT sum_cargo FROM aux_saldo1.

  IF aux_saldo1 NE up_salfin.
*    MESSAGE e000 WITH 'ERROR EN CONTROL DE SALDOS'.
  ENDIF.
  up_varnum      = sum_cargo.
  wa_cab-tot_c   = up_varnum.

  up_varnum      = sum_abono.
  wa_cab-tot_a   = up_varnum.

  up_varnum      = up_salfin.
  wa_cab-sal_fin = up_varnum.
...
*            WITH SY-MSGV1 SY-MSGV2 SY-MSGV3 SY-MSGV4.
    ELSE.
*Begin of change: ReSQ Correction INDEX READ on an unsorted Internal TABLE 19/12/2019 EY_DES04 ECDK917080 *
*SORT ITAB_FILES . "JOROZCO 20.01.2020
*End of change: ReSQ Correction for INDEX READ on an unsorted Internal TABLE 19/12/2019 EY_DES04 ECDK917080 *
