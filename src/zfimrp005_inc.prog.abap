...
* NEW CODE
        ...
        WHERE ICHEC EQ ''
          ...
          AND HBKID EQ HBKID
          AND HKTID EQ HKTID
          AND VBLNR EQ T_BSIS-BELNR ORDER BY PRIMARY KEY.
...
* NEW CODE
       ...
       WHERE ICHEC EQ ''
         ...
         AND HBKID EQ HBKID
         AND HKTID EQ HKTID
         AND CHECT EQ T_BSIS-ZUONR ORDER BY PRIMARY KEY.
...
* NEW CODE
    ...
    UP TO 1 ROWS  FROM APQI
        WHERE  GROUPID EQ GROUP
           AND  QSTATE  EQ ' '
           AND  USERID  EQ SY-UNAME
           AND  CREDATE EQ SY-DATUM ORDER BY PRIMARY KEY.
...
* NEW CODE
          ...
          UP TO 1 ROWS  FROM APQI
              WHERE QID     EQ QID
              AND   USERID  EQ SY-UNAME
              AND   GROUPID EQ GROUP ORDER BY PRIMARY KEY.
...
* NEW CODE
      SELECT AUGBL
      UP TO 1 ROWS  INTO P_DOC
        FROM BSAS
        WHERE BUKRS EQ T_OK-BUKRS
          AND HKONT EQ T_OK-HKONT
*    AND ZUONR EQ T_OK-CHECT
          AND GJAHR EQ T_OK-GJAHR
          AND BELNR EQ T_OK-BELNR ORDER BY PRIMARY KEY.
...
* NEW CODE
  ...
  UP TO 1 ROWS 
    FROM ZCTA_PRESCRIP
    INTO G_HKOD
   WHERE T_CUENTA EQ 4 ORDER BY PRIMARY KEY.
...
* NEW CODE
        ...
        UP TO 1 ROWS 
          FROM ZMOT_PRESCRIP
          INTO P_DIAS
         WHERE ZZMOT_EMIS EQ P_ZMOT_EMIS ORDER BY PRIMARY KEY.
...
* NEW CODE
  SELECT ZZMOT_EMIS
  UP TO 1 ROWS  INTO MOT_EMIS
    FROM BSEG
   WHERE BUKRS EQ BUKRS
     AND BELNR EQ P_BELNR
     AND GJAHR EQ GJAHR
     AND BUZEI EQ 1
     AND KOART EQ 'S' ORDER BY PRIMARY KEY.
