...
****    LOOP AT t_ok.
****      v_index = sy-tabix.
****      SELECT SINGLE jdatos FROM zjdatos_edocheq INTO t_ok-jdatos
...
****  fieldcat_ln-seltext_l = 'Secuencia'.
****  fieldcat_ln-hotspot = ' '.
****  APPEND fieldcat_ln TO gt_fieldcat.
***** fin FCV - 07.08.2010
****
...
****    juego_datos = 'CC'.
****  ENDIF.
****  CONCATENATE juego_datos sy-datum+6(2) sy-datum+4(2) sy-datum+2(2) sy-uzeit(4) INTO juego_datos.
****  group = juego_datos.
****
****  bkpf-budat = p_budat.
****
...
************************************************************************************
****    ENDIF.
***** Fin modificación FCV - 24.06.2010
****  ENDIF.
****
***** chidalgo - Quintec 27.05.2010
***** Agregar columna de juego de datos
...
****    CLEAR: v_primera, v_correlativo, nro_secuencia.
****    LOOP AT t_ok.
***** FCV - 22.04.2010
...
****    IF t_ok-zmote = 'SUBMATERNA' AND t_ok-estado = 'REVALIDADO'.
...
****                                       USING bukrs hbkid hktid bkpf-budat.
***** FCV - 06.08.2010
****    LOOP AT t_ok WHERE zmote = 'SUBMATERNA'.
****      v_puntero = sy-tabix.
****      IF t_ok-estado = 'CHEQUE GIRADO'.
****        MOVE '@08@'    TO t_ok-status. " ICONO BIEN
****      ELSE.
...
****                 AND box EQ 'X'
****                 AND zmote = 'SUBMATERNA'.
****
****    v_correlativo = v_correlativo + 1.
****    nro_secuencia = nro_secuencia + 1.
****    CLEAR: v_xblnr, v_monto.
****    WRITE t_ok-wrbtr TO v_monto CURRENCY 'CLP'.
****    CONCATENATE sy-datum+6(2) sy-datum+4(2) sy-datum+2(2) sy-uzeit+0(6) v_correlativo
****              INTO v_xblnr.
****
****    t_ftpost-stype = 'K'.
****    t_ftpost-count = '1'.
