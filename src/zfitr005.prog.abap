...
*& Date: <15-11-2022> *
*& Mofificación : incorpora la sociedad CL27 igual a CL13
*& Transport Number: < ECDK923143 > *
*&---------------------------------------------------------------------*
REPORT zfitr005 NO STANDARD PAGE HEADING
                                   LINE-COUNT 60
                                   LINE-SIZE 132 .
...
*ReSQ: No Need Of Change Internal Table LT_DYNPFIELDS Already Sorted
  ...
  CONCATENATE 'C:\TRANSFER\' bukrs '_BBCI' '_' sy-datum '_' sy-uzeit '.txt' INTO
...
INITIALIZATION.
  ...
  CONCATENATE archivo 'BBCI' '_' sy-datum '_' sy-uzeit '.txt' INTO
  archivo.
...
* NEW CODE
    ...
    ENDSELECT.
* END. 07-07-2026 - ATC - ATC-01
    bktxtasi  =  bktxt. " HCD 25052024" HCD 20210122 se solicita que texto de asignacion se coloque en el comprobante
...
** V1 RVY 24-01-2023
** V1 MJD 27-03-2023 - Inicio.
  CLEAR reg_stder-mail.
...
*  Primero busca la dirección por defeccto.
* BEGIN. 07-07-2026 - ATC - ATC-01
* OLD CODE
*    SELECT SINGLE smtp_addr INTO reg_stder-mail
*            FROM adr6 WHERE addrnumber = tabla_00-adrnr AND
*                            persnumber EQ space  AND
*                            flgdefault EQ 'X'.
*
* NEW CODE
    SELECT smtp_addr
    UP TO 1 ROWS  INTO reg_stder-mail
            FROM adr6 WHERE addrnumber = tabla_00-adrnr AND
                            persnumber EQ space  AND
                            flgdefault EQ 'X' ORDER BY PRIMARY KEY.

    ENDSELECT.
* END. 07-07-2026 - ATC - ATC-01
* Si no encuentra una, busca la primera disponible, si es que existe.
    IF sy-subrc <> 0.
* BEGIN. 07-07-2026 - ATC - ATC-01
* OLD CODE
*      SELECT SINGLE smtp_addr INTO reg_stder-mail
...
* NEW CODE
      ...
      UP TO 1 ROWS  INTO reg_stder-mail
              FROM adr6 WHERE addrnumber = tabla_00-adrnr AND
                              persnumber EQ space ORDER BY PRIMARY KEY.

      ENDSELECT.
* END. 07-07-2026 - ATC - ATC-01
    ENDIF.
** V1 MJD 27-03-2023 - Fin.
  ENDIF.
  ...
  IF tabla_00-rzawe  = 'V'.
    IF tabla_00-zbukr = 'CL13' OR tabla_00-zbukr = 'CL17' OR
** V1 INI RVY 15-11-22
       tabla_00-zbukr = 'CL15'.
** V1 FIN RVY 15-11-22
    ...
    ENDIF.
    reg_stder-codigo_banco = 016.
    reg_stder-suc_retiro = 245.
** HCD INI CAMBIO 04-01-2023 se cambia codigo de retiro para Vespucio
    IF     tabla_00-zbukr = 'CL27' OR       tabla_00-zbukr = 'CL28'.
      reg_stder-suc_retiro = 103.
    ENDIF.
** HCD FIN CAMBIO 04-01-2023 se cambia codigo de retiro para Vespucio
  ELSE.
* BEGIN. 07-07-2026 - ATC - ATC-01
* OLD CODE
...
* END. 07-07-2026 - ATC - ATC-01
  ...
  IF  tabla_00-rzawe  = 'T'.
    ...
    ENDIF.
    IF tabla_00-zbukr = 'CL13' OR tabla_00-zbukr = 'CL17' OR tabla_00-zbukr = 'CL27' OR
       tabla_00-zbukr = 'CL28' OR
** V1 INI RVY 15-11-22
       tabla_00-zbukr = 'CL15' OR tabla_00-zbukr = 'CL12' OR tabla_00-zbukr = 'CL16' OR
** V1 FIN RVY 15-11-22
** V1 INI RVY 17-10-23
**     tabla_00-zbukr = 'CL65'.
       tabla_00-zbukr = 'CL65' OR tabla_00-zbukr = 'CL19'. .
      IF reg_stder-codigo_banco = 016 OR reg_stder-codigo_banco = 509 OR reg_stder-codigo_banco = 050.
  ...
  " INI Se deja comentareado 04-09-2020
  "INI HCD 20200826
  " IF  tabla_00-rzawe  = 'T' and reg_stder-codigo_banco = 016.
  "    IF tabla_00-zbukr = 'CL27' or tabla_00-zbukr = 'CL28'.
  "        reg_stder-mod_pago  = 'VVC'.
  "    ENDIF.
  " ENDIF.
  " FIN  Se deja comentareado 04-09-2020
  "FIN CD 20200826
****Es multisociedad?
...
** INI V1 RVY 30-11-2022
*  IF bukrs = 'CL27'.
  IF bukrs = 'CL27' OR bukrs = 'CL13' OR
     bukrs = 'CL15' OR bukrs = 'CL17'.
** FIN V1 RVY 30-11-2022
...
TOP-OF-PAGE.
  WRITE : /1  t001-butxt,
           43   'NOMINA DE PAGO PROVEEDORES CUENTA BANCO BCI' ,
           120 sy-datum,
  ...
  LOOP AT reg_stder.
    ...
    IF reg_stder-mod_pago  = 'VVC'.
      CASE reg_stder-blart.
        ...
        WHEN 'S1' OR 'SA' OR 'SB' OR 'F7' OR 'AB'." HCD 09-01-2023
          out_reg_stder-tipo_docto = 'ABO'. " HCD 09-01-2023
      ENDCASE.
    ELSE." SE AGREGA 04-09-2020
      IF tabla_00-zbukr = 'CL13' OR tabla_00-zbukr = 'CL17' OR tabla_00-zbukr = 'CL27' OR tabla_00-zbukr = 'CL28' OR
** V1 INI RVY 15-11-22
         tabla_00-zbukr = 'CL15'.
** V1 FIN RVY 15-11-22
        CASE reg_stder-blart.
          WHEN 'F0' OR 'F1' OR 'F2' OR 'F3' OR 'F4' OR 'F5' OR 'F6'  OR 'F9'.
            out_reg_stder-tipo_docto = 'FAC'.
          WHEN 'FA' OR 'FB' OR 'FC' OR 'FR' OR 'I1' OR 'I2'.
            out_reg_stder-tipo_docto = 'FAC'.
          WHEN 'B1' OR 'B2' OR 'B3' OR 'B4' OR 'B5'.
            out_reg_stder-tipo_docto = 'ABO'.
          WHEN 'N0' OR 'N1' OR 'N2' OR 'N3' OR 'N4' OR 'NA' OR 'NB' OR 'NC' OR 'NR'.
            out_reg_stder-tipo_docto = 'NCR'.
          WHEN 'D1' OR 'D2' OR 'D3' OR 'D4'.
            out_reg_stder-tipo_docto = 'NDB'.
          WHEN 'S1' OR 'SA' OR 'SB' OR 'F7' OR 'AB'.
            out_reg_stder-tipo_docto = 'ABO'.
        ENDCASE.
      ELSE.
        out_reg_stder-tipo_docto = 'ABO'.
...
** Fin V1 RVY 30-11-2022
** INI V1 RVY 17-10-23
      bukrs = 'CL15' OR bukrs = 'CL17' OR bukrs = 'CL19'.
    ...
    ENDIF.
** Ini V1 RVY 30-11-2022
*  ELSEIF bukrs = 'CL17' or  bukrs = 'CL13' OR  " Se agrega esta opcion para DAVILA Y SIS HCD 20120424
*** V1 INI RVY 15-11-22
*         bukrs = 'CL15'.
*** V1 FIN RVY 15-11-22
...
* CBD - 2012-01-11 - MANEJO CTA CARGO -                               cuenta.
  ...
  PERFORM bdc_field       USING 'BSEG-SGTXT'
                                bktxt.
  PERFORM bdc_field       USING 'BSEG-ZUONR'
                                bktxtasi."bktxt." se cambia xblnr. por bktxt por solicitur de usuario HCD 20200122 HCD27052024
