 ...
 INTO CORRESPONDING FIELDS OF TABLE t_payr
    FROM payr
     WHERE ichec EQ ''
       ...
       AND hktid EQ hktid
       AND chect IN psel
       AND zaldt IN pfepag ORDER BY PRIMARY KEY.
...
* NEW CODE
    SELECT *
    UP TO 1 ROWS  INTO CORRESPONDING FIELDS OF  t_bsis
      FROM bsis
       WHERE bukrs EQ t_payr-zbukr
         AND hkont EQ t_payr-ubhkt
         AND gjahr EQ t_payr-gjahr
...
* NEW CODE
  SELECT * APPENDING  CORRESPONDING FIELDS OF TABLE  t_bsis
    ...
    FROM bsis
     WHERE bukrs = bukrs
       AND hkont IN t_cta ORDER BY PRIMARY KEY.
...
* NEW CODE
                SELECT *
                UP TO 1 ROWS  FROM zprescribe_fecha
                  WHERE bukrs = t_ok-bukrs
                    AND hbkid = hbkid
                    AND hktid = hktid
...
* NEW CODE
          ...
          UP TO 1 ROWS  INTO zcambiocheque-xblnr
            ...
            WHERE zbukr = t_bseg-bukrs
              ...
              AND hktid = wa_payr-hktid
              AND rzawe = 'C'
              AND chect = t_bseg-zuonr ORDER BY PRIMARY KEY.
...
* NEW CODE
      SELECT *
      UP TO 1 ROWS  FROM zrangos_prescri
        WHERE motivoemision = t_ok-zmote ORDER BY PRIMARY KEY.
...
* NEW CODE
      SELECT cuenta_p
      UP TO 1 ROWS  INTO g_hkod
      FROM zcta_prescrip
      WHERE t_cuenta = 4 ORDER BY PRIMARY KEY.
...
* END. 07-07-2026 - ATC - ATC-03
  ...
  LOOP AT t_ok.
    ...
    v_puntero = v_index.
    TRANSLATE t_ok-zmote TO UPPER CASE.
    IF t_ok-status <> '@0A@' AND t_ok-zmote = 'SUBMATERNA'.  " No se permite revalidar el motivo
      t_ok-status =  '@0A@'.
      MODIFY t_ok INDEX v_index.
    ENDIF.
...
* NEW CODE
    ...
    UP TO 1 ROWS 
      FROM zcta_prescrip
      INTO g_hkod
     WHERE t_cuenta EQ 6 ORDER BY PRIMARY KEY.
