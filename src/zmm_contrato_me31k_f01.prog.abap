...
*
  LOOP AT gt_table ASSIGNING FIELD-SYMBOL(<campos>).
    ...
    <campos>-ematn = |{ <campos>-ematn ALPHA = IN }|.
*
...
*
    SELECT SINGLE matnr INTO @DATA(lv_matnr)
           FROM mara WHERE matnr EQ @<campos>-ematn.
    IF sy-subrc NE 0.
...
*
  LOOP AT p_gt_fieldcat ASSIGNING FIELD-SYMBOL(<fieldcat>).
    CASE <fieldcat>-fieldname.
      WHEN 'CLAVE'.
        <fieldcat>-scrtext_m = 'Clave'.
        <fieldcat>-key       = gc_x.
      WHEN 'KDATB'.
        <fieldcat>-scrtext_m = 'In.período validez'.
      WHEN 'KDATE'.
        <fieldcat>-scrtext_m = 'Fin período validez'.
      WHEN 'CLAVE_REF'.
        <fieldcat>-scrtext_m = 'Clave Posición'.
      WHEN 'POSIC_REF'.
        <fieldcat>-scrtext_m = 'Posición'.
      WHEN 'KTMNG'.
        <fieldcat>-scrtext_m = 'Cantidad prevista'.
      WHEN 'NETPR'.
        <fieldcat>-scrtext_m = 'Precio neto'.
      WHEN 'EBELN'.
        <fieldcat>-hotspot   = gc_x.
      WHEN 'MEINS'.
        <fieldcat>-scrtext_m = 'Unidad Medida'.
    ENDCASE.
**
*    <fieldcat>-scrtext_s = <fieldcat>-scrtext_m.
*    <fieldcat>-scrtext_l = <fieldcat>-scrtext_m.
...
*&---------------------------------------------------------------------*
FORM crear_contrato .
  DATA : lt_table    TYPE TABLE OF zes_me31k,
         lt_itab     TYPE TABLE OF bdcmsgcoll,
         bdcdata_tab TYPE TABLE OF bdcdata,
         lw_opt      TYPE ctu_params,
         wa_campos   TYPE zes_me31k,
         lv_docum    TYPE i,
         lv_num      TYPE char02,
         lv_msg      TYPE bapi_msg.
*
  lt_table[] = gt_table[].
  DELETE ADJACENT DUPLICATES FROM lt_table COMPARING clave.
  LOOP AT lt_table INTO wa_campos WHERE ebeln IS INITIAL.
    CLEAR bdcdata_tab[].
    bdcdata_tab = VALUE #(
      ( program  = 'SAPMM06E' dynpro   = '0200' dynbegin = 'X' )
      ( fnam     = 'BDC_OKCODE'        fval = '/00'           )
      ( fnam     = 'EKKO-LIFNR'        fval = wa_campos-lifnr )
      ( fnam     = 'RM06E-EVART'       fval = wa_campos-evart )
      ( fnam     = 'EKKO-EKORG'        fval = wa_campos-ekorg )
      ( fnam     = 'EKKO-EKGRP'        fval = wa_campos-ekgrp )
*
      ( program  = 'SAPMM06E' dynpro   = '0514' dynbegin = 'X' )
      ( fnam     = 'BDC_OKCODE'        fval = '=ENTE'         )
      ( fnam     = 'EKKO-BUKRS'        fval = wa_campos-bukrs )
*
      ( program  = 'SAPMM06E' dynpro   = '0201' dynbegin = 'X' )
      ( fnam     = 'BDC_OKCODE'        fval = '=AB'           )
      ( fnam     = 'EKKO-KDATB'        fval = wa_campos-kdatb )
      ( fnam     = 'EKKO-KDATE'        fval = wa_campos-kdate )
      ( fnam     = 'EKKO-WAERS'        fval = wa_campos-waers )
       ).
*
    lv_num = '01'.
    LOOP AT gt_table INTO DATA(lw_table) WHERE clave_ref EQ wa_campos-clave.

      bdcdata_tab = VALUE #( BASE bdcdata_tab
        ( program  = 'SAPMM06E' dynpro   = '0220' dynbegin = 'X' )
        ( fnam     = 'BDC_OKCODE'                    fval = '/00'           )
        ( fnam     = 'EKPO-EMATN(' && lv_num && ')'  fval = lw_table-ematn )
        ( fnam     = 'EKPO-KTMNG(' && lv_num && ')'  fval = lw_table-ktmng )
        ( fnam     = 'EKPO-MEINS(' && lv_num && ')'  fval = lw_table-meins )
        ( fnam     = 'EKPO-NETPR(' && lv_num && ')'  fval = lw_table-netpr )
*
        ( program  = 'SAPMM06E' dynpro   = '0220' dynbegin = 'X' )
        ( fnam     = 'BDC_OKCODE'                    fval = '=NP'           )
        ( fnam     = 'EKPO-KTMNG(' && lv_num && ')'  fval = lw_table-ktmng )
        ( fnam     = 'EKPO-NETPR(' && lv_num && ')'  fval = lw_table-netpr )
         ).
      lv_num = '02'.
...
*
    ...
    TRY.
        CLEAR lt_itab[].
        CALL TRANSACTION 'ME31K' WITH AUTHORITY-CHECK
          ...
          LOOP AT lt_itab INTO DATA(lw_itab) WHERE  msgtyp = 'E'.
            MESSAGE ID lw_itab-msgid TYPE lw_itab-msgtyp NUMBER lw_itab-msgnr
                    WITH lw_itab-msgv1 lw_itab-msgv2 lw_itab-msgv3 lw_itab-msgv4
                         INTO lv_msg.
          ENDLOOP.
          LOOP AT gt_table ASSIGNING FIELD-SYMBOL(<lw_table>)
                                       WHERE clave_ref EQ wa_campos-clave.
            <lw_table>-mensaje  = lv_msg.
            EXIT.
          ENDLOOP.
        ELSE.
          LOOP AT lt_itab INTO lw_itab WHERE msgtyp EQ 'S'
                                         AND msgid  EQ '06'
                                         AND msgnr  EQ '017'
                                         AND msgv2 IS NOT INITIAL.
            DATA(lv_ebeln) = lw_itab-msgv2.
            EXIT.
          ENDLOOP.
          LOOP AT gt_table ASSIGNING <lw_table>
                                       WHERE clave_ref EQ wa_campos-clave.
            <lw_table>-ebeln = lv_ebeln.
            ADD 1 TO lv_docum.
            EXIT.
          ENDLOOP.
        ENDIF.
  ...
  ENDLOOP.
*
  DESCRIBE TABLE  lt_table LINES sy-tabix.
  IF sy-tabix EQ lv_docum..
    gv_proc = gc_x.
  ENDIF.
