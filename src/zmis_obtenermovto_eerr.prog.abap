...
types: begin of est_salida,
        ...
        zzrut_terc type bseg-zzrut_terc,  "Id Deudor
...
data: it_salida type standard table of est_salida with header line,
      ...
      fecha_borra_i(8) type c,
      fecha_borra_f(8) type c,
      fanoini(4) type c,
      fanofin(4) type c,
      fmesini(2) type c,
      fmesfin(2) type c,
...
r_hkont-sign = 'I'.
r_hkont-option = 'BT'.
r_hkont-low = '4100000000'.
r_hkont-high = '5511110000'.
append r_hkont.
...
*        total_impto = 0.
*        loop at it_bset.
*          total_neto = total_neto + it_bset-hwbas.
*          total_impto = total_impto + it_bset-hwste.
*        endloop.
*      else.
*        total_neto = 0.
*        total_impto = 0.
...
*      wa-hwbas = total_neto.
*      wa-hwste = total_impto.
      wa-hwbas = 0.
      wa-hwste = 0.
      wa-origen = it_bkpf-xref2_hd.
      wa-rldnr = it_bkpf-rldnr.
...
loop at it_salida.
  exec sql.
    insert into sap_eerr (sociedad,comprobante,fecha,referencia,fecha_doc,tipo_doc,cuenta,ccosto,glosa,
                          zzprestac,zzunid_pro,zzdesc_est,zzmot_emis,zzrut_terc,zzrut_prov,zznom_prov,zz_agencia,sentido,valor,valor_neto,valor_impto,origen,ledger)
    values (:it_salida-bukrs,:it_salida-belnr,:it_salida-budat,:it_salida-xblnr,:it_salida-bldat,:it_salida-blart,:it_salida-hkont,:it_salida-prctr,:it_salida-sgtxt,
            ...
            :it_salida-zz_agencia,:it_salida-shkzg,:it_salida-dmbtr,:it_salida-hwbas,:it_salida-hwste,:it_salida-origen,:it_salida-rldnr)
  endexec.
endloop.
