...
* NEW CODE
            SELECT *
            UP TO 1 ROWS  FROM  IDBUPLA_ACTIVE INTO
              WA_BUPLA_ACTIVE WHERE LAND1 = COUNTRY ORDER BY PRIMARY KEY.
...
* NEW CODE
      SELECT *
      UP TO 1 ROWS  FROM T007A                            "Note532443
         WHERE MWSKZ = BSEG-MWSKZ                           "Note532443
           AND KALSM = T005-KALSM ORDER BY PRIMARY KEY.                          

      ENDSELECT.
* END. 07-07-2026 - ATC - ATC-01"Note532443
...
* NEW CODE
      SELECT BLART
 FROM  T003 INTO T003-BLART               "Note 496812
             WHERE  XKOAK  = 'X'
             AND    BLART IN PP_BLART ORDER BY PRIMARY KEY.
...
* NEW CODE
      SELECT BLART
 FROM  T003 INTO T003-BLART               "Note 496812
             WHERE  XKOAD  = 'X' ORDER BY PRIMARY KEY.
...
* NEW CODE
  SELECT LTEXT
  UP TO 1 ROWS  FROM  T003T INTO T003T-LTEXT
         WHERE  SPRAS  = LANGU
         AND    BLART  = BLART ORDER BY PRIMARY KEY.
