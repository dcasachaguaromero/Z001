...
DATA : BEGIN OF reg_stdis OCCURS 0,
          codigo_banco(03)    TYPE n ,
          cuenta_abono(18)    TYPE c ,
          monto type p,
          rut_prov(9)        TYPE c ,
...
DATA : BEGIN OF reg_stdis2 OCCURS 0,
          codigo_banco(03)    TYPE n ,
          cuenta_abono(18)    TYPE c ,
          monto(10) type n,
          rut_prov(9)        TYPE c ,
...
FIELD-SYMBOLS : <nombre1_> TYPE ANY ,
                <nombre2_> TYPE ANY ,
                <nombre3_> TYPE ANY .
*  se cambia  lins(04) a lins(06)     TYPE c VALUE 0, HCD 26-04-2018
DATA:  nombre1(25)          ,
       nombre2(25)          ,
       nombre3(25)          ,
       ndx(02)      TYPE n  ,
       lins(06)     TYPE c VALUE 0,
       num_doc(8)   TYPE c  .
...
* BEGIN. 07-07-2026 - ATC - ATC-01
* OLD CODE
*  select single UBNKY from reguh into vbankl
*                 where laufd       = V_fecha
*                 and   laufi       = V_nomina
*                 and   ZBUKR       = bukrs.
*
* NEW CODE
  SELECT UBNKY
  UP TO 1 ROWS  from reguh into vbankl
                 where laufd       = V_fecha
                 and   laufi       = V_nomina
...
*--------------------*
  ...
  data: ceros18(18) type c value '000000000000000000'.
  data: cuenta(18) type c value ''.
...
*    ENDLOOP.
    SELECT *  FROM REGUP      CLIENT SPECIFIED
           WHERE  MANDT       = SY-MANDT
             ...
             AND  XVORL       = ' '
             AND  LIFNR       = ACREEDOR
*Begin of change: ReSQ Correction for Addition ORDER BY PRIMARY KEY 27/12/2019 EY_DES01 ECDK916962*
*             AND  VBLNR       = TABLA_00-VBLNR.
             AND  VBLNR       = TABLA_00-VBLNR ORDER BY PRIMARY KEY.
*End of change: ReSQ Correction for Addition ORDER BY PRIMARY KEY 27/12/2019 EY_DES01 ECDK916962*
...
*-----------------*
  ...
  LOOP AT reg_stder.
    IF reg_stder-signo_docto01 = '+'.
      hsum = hsum + reg_stder-monto_docto01.
    ELSE.
      hsum = hsum - reg_stder-monto_docto01.
    ENDIF.
    IF reg_stder-signo_docto02 = '+'.
      hsum = hsum + reg_stder-monto_docto02.
    ELSE.
      hsum = hsum - reg_stder-monto_docto02.
    ENDIF.
    IF reg_stder-signo_docto03 = '+'.
      hsum = hsum + reg_stder-monto_docto03.
    ELSE.
      hsum = hsum - reg_stder-monto_docto03.
    ENDIF.
    IF reg_stder-signo_docto04 = '+'.
      hsum = hsum + reg_stder-monto_docto04.
    ELSE.
      hsum = hsum - reg_stder-monto_docto04.
    ENDIF.
    IF reg_stder-signo_docto05 = '+'.
      hsum = hsum + reg_stder-monto_docto05.
    ELSE.
      hsum = hsum - reg_stder-monto_docto05.
    ENDIF.
    IF reg_stder-signo_docto06 = '+'.
      hsum = hsum + reg_stder-monto_docto06.
    ELSE.
      hsum = hsum - reg_stder-monto_docto06.
    ENDIF.
    IF reg_stder-signo_docto07 = '+'.
      hsum = hsum + reg_stder-monto_docto07.
    ELSE.
      hsum = hsum - reg_stder-monto_docto07.
    ENDIF.
    IF reg_stder-signo_docto08 = '+'.
      hsum = hsum + reg_stder-monto_docto08.
    ELSE.
      hsum = hsum - reg_stder-monto_docto08.
    ENDIF.
    IF reg_stder-signo_docto09 = '+'.
      hsum = hsum + reg_stder-monto_docto09.
    ELSE.
      hsum = hsum - reg_stder-monto_docto09.
    ENDIF.
    IF reg_stder-signo_docto10 = '+'.
      hsum = hsum + reg_stder-monto_docto10.
    ELSE.
      hsum = hsum - reg_stder-monto_docto10.
    ENDIF.
    IF reg_stder-signo_docto11 = '+'.
      hsum = hsum + reg_stder-monto_docto11.
    ELSE.
      hsum = hsum - reg_stder-monto_docto11.
    ENDIF.
    CONDENSE reg_stder-codigo_banco NO-GAPS.
    reg_stdis-codigo_banco = reg_stder-codigo_banco.
*    CONDENSE reg_stder-cuenta_abono NO-GAPS.
    if reg_stder-codigo_banco = '012'.
          translate reg_stder-cuenta_abono using 'K0'.
    endif.
    if reg_stder-codigo_banco = '049'.
          translate reg_stder-cuenta_abono using 'E0'.
    endif.
    reg_stdis-cuenta_abono = reg_stder-cuenta_abono.
    reg_stdis-monto = hsum.
    CONDENSE reg_stder-rut_prov NO-GAPS.
    reg_stdis-rut_prov = reg_stder-rut_prov.
    CONDENSE reg_stder-nombre.
    reg_stdis-nombre = reg_stder-nombre.
    COLLECT reg_stdis.
    CLEAR hsum.
  ENDLOOP.

  LOOP AT reg_stdis.
    reg_stdis2-codigo_banco = reg_stdis-codigo_banco.
    reg_stdis2-cuenta_abono = reg_stdis-cuenta_abono.
    reg_stdis2-monto = reg_stdis-monto.
    SHIFT reg_stdis-rut_prov RIGHT DELETING TRAILING SPACE. " If U want leading 0s
    OVERLAY reg_stdis-rut_prov WITH '000000000'.
    reg_stdis2-rut_prov = reg_stdis-rut_prov.
    reg_stdis2-nombre = reg_stdis-nombre.
    APPEND reg_stdis2.
  ENDLOOP.
