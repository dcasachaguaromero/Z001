...
* NEW CODE
    SELECT stblg
    UP TO 1 ROWS  INTO ls_stblg     "mod 24.07.2015 PCM descarta anulados MM
      FROM rbkp                           "mod 24.07.2015 PCM descarta anulados MM
      WHERE belnr = ls_bkpf-awkey(10)     "mod 24.07.2015 PCM descarta anulados MM
        AND gjahr = ls_bkpf-awkey+10(4) ORDER BY PRIMARY KEY.  

    ENDSELECT.
* END. 07-07-2026 - ATC - ATC-01"mod 24.07.2015 PCM descarta anulados MM
...
* NEW CODE
  ...
  UP TO 1 ROWS 
    FROM lfa1
    INTO (ps_outtab-name1, ps_outtab-stcd1)
    WHERE lifnr EQ ps_bseg-lifnr ORDER BY PRIMARY KEY.
...
* NEW CODE
    SELECT *
     ...
     WHERE bukrs EQ ps_bset-bukrs
       AND belnr EQ ps_bset-belnr
       AND gjahr EQ ps_bset-gjahr
       AND mwskz NE 'C0' ORDER BY PRIMARY KEY.
...
* NEW CODE
 ...
 FROM bkpf
          WHERE bukrs = ps_bset-bukrs
                AND belnr = ps_bset-belnr
                AND gjahr = ps_bset-gjahr ORDER BY PRIMARY KEY.
...
* NEW CODE
 ...
 FROM bseg
          WHERE bukrs = ps_bset-bukrs
                ...
                AND shkzg <> ps_bset-shkzg
                AND mwskz = ps_bset-mwskz
                AND txgrp = ps_bset-txgrp ORDER BY PRIMARY KEY.
...
* NEW CODE
  SELECT wt_qbshh
  UP TO 1 ROWS  INTO ps_outtab-taxbash6
    FROM with_item
      WHERE bukrs = ps_bkpf-bukrs
        AND belnr = ps_bkpf-belnr
        AND gjahr = ps_bkpf-gjahr
        AND witht = 'Q4'
        AND wt_withcd = 'B4' ORDER BY PRIMARY KEY.
...
* NEW CODE
      SELECT *
      UP TO 1 ROWS  FROM zprov_lce_buk WHERE bukrs = p_bukrs ORDER BY PRIMARY KEY.
...
* NEW CODE
        SELECT *
        UP TO 1 ROWS  FROM zprov_lce WHERE codprov = zprov_lce_buk-codprov ORDER BY PRIMARY KEY.
