...
*&
*&---------------------------------------------------------------------*
*&Rescata Tabla ANLP Original Respaldada
*&
*&---------------------------------------------------------------------*
...
data : begin of t_texto occurs 0,
       ...
       begin of t_anlp occurs 0.
        include structure anlp.
data :    box   type c,
       end of t_anlp,
       err_anlp like anlp occurs 0 with header line,
       ERROR    TYPE C.
* Variables ALV
TYPE-POOLS slis.
...
FORM user_command USING r_ucomm     LIKE sy-ucomm
  ...
  case r_ucomm.
    when 'FC01'.
      refresh err_anlp.
      LOOP AT T_ANLP WHERE BOX EQ 'X'.
        MOVE SY-TABIX TO L_TABIX.
        move-corresponding t_anlp to anlp.
* PGR CARGA EN AREA ALTERNATIVA
        IF NOT P_AF_ALT IS INITIAL.
          move P_AF_ALT to anlp-afaber.
        ENDIF.
***********************************
*        insert anlp.
* PGR
* Asigna doc.Unico para valores procesados en otro sistema
*        anlp-belnr = 999 .
        check p_test is initial.
        MODIFY ANLP.
        if sy-subrc ne 0.
          append anlp to err_anlp.
        endif.
        T_ANLP-BOX = ' '.
        MODIFY T_ANLP INDEX L_TABIX.
      ENDLOOP.
      IF SY-SUBRC NE 0.
...
*    ls_line-info = 'Datos de tabla ANLP Respaldados'.
*  ELSE.
*    ls_line-info = 'Datos con ERROR de tabla ANLP Respaldados'.
*  ENDIF.
...
*    anlp-belnr = 999 .
    ...
    MODIFY ANLP.
    if sy-subrc ne 0.
      write:/ 'Error al Grabar' , ANLP-anln1, ANLP-anln2.
    endif.
  ENDLOOP.
*
ENDFORM.                    " ACTUALIZA_DATOS_BATCH
