...
* NEW CODE
  SELECT *
    ...
    INTO CORRESPONDING FIELDS OF TABLE lt_dte_doc_rec
    FROM zdte_doc_rec
   WHERE estado IN r_estado ORDER BY PRIMARY KEY.
...
* NEW CODE
      ...
      INTO wa_listabcab
      FROM zbmnc_listabcab
      WHERE rut_proveedor EQ wa_doc_rec-rutemisor ORDER BY PRIMARY KEY.
...
* NEW CODE
        SELECT *
        UP TO 1 ROWS  INTO lv_lfa1
          FROM lfa1
         WHERE lifnr EQ ti_cab-lifnr ORDER BY PRIMARY KEY.
...
* NEW CODE
        ...
        UP TO 1 ROWS 
          FROM lfb1
         WHERE lifnr EQ ti_cab-lifnr
           AND bukrs EQ lv_bukrs ORDER BY PRIMARY KEY.
...
* NEW CODE
        ...
        UP TO 1 ROWS  INTO  iva_prop
          ...
          WHERE  bukrs EQ lv_bukrs
          AND    fec_inico <= sy-datum
          AND    fec_fin  >= sy-datum ORDER BY PRIMARY KEY.
...
* NEW CODE
  ...
  UP TO 1 ROWS  INTO w_bkpf
   ...
   WHERE bukrs EQ l_bukrs
     AND belnr EQ l_belnr
     AND gjahr EQ l_gjahr ORDER BY PRIMARY KEY.
