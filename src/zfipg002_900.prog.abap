...
*&---------------------------------------------------------------------*
*&      Module  VALIDA_BLART  OUTPUT
*&---------------------------------------------------------------------*
MODULE valida_blart OUTPUT.
  DATA : lr_blart TYPE RANGE OF bkpf-blart.
  CONSTANTS lc_name TYPE c LENGTH 30 VALUE 'ZFI_ZFIPG002_BLART'.
*
  DATA(lv_blart) = int_tabla3[ 1 ]-blart.
*
  SELECT sign opti low high INTO TABLE lr_blart
        FROM tvarvc WHERE name EQ lc_name
                    AND   type EQ 'S'.
*
  IF lv_blart IN lr_blart AND lr_blart[] IS NOT INITIAL.
    DATA(lv_valida) = 'X'.
    ref = 'X'.
    CLEAR : asig, rese.
  ELSE.
    CLEAR : lv_valida, ref, rese.
    asig = 'X'.
  ENDIF.
*
  LOOP AT SCREEN.
