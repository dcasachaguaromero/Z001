...
* NEW CODE
  ...
  FROM  reguh
           ...
           AND    xvorl      = ' '
           AND    ubnkl      = '027'
           AND  ( rzawe      = 'C' ) ORDER BY PRIMARY KEY. 

* END. 07-07-2026 - ATC - ATC-03" OR rzawe = 'V' ).
...
* BEGIN. 07-07-2026 - ATC - ATC-01
* OLD CODE
*    SELECT SINGLE chect FROM  payr INTO t_doctos-chect WHERE zbukr  = bsak-bukrs
*                               AND   vblnr  = bsak-augbl
*                               AND   gjahr  = bsak-auggj.
*
* NEW CODE
    SELECT chect
    UP TO 1 ROWS  FROM  payr INTO t_doctos-chect WHERE zbukr  = bsak-bukrs
                               AND   vblnr  = bsak-augbl
                               AND   gjahr  = bsak-auggj ORDER BY PRIMARY KEY.
