...
* NEW CODE
  SELECT lifnr
  UP TO 1 ROWS  INTO lifnr FROM vf_kred
                                 WHERE  bukrs = bukrs
...
* NEW CODE
      SELECT *
      UP TO 1 ROWS  FROM bsik WHERE lifnr = lifnr
                                AND bukrs   =  bukrs
                                AND gjahr   =  bsip-gjahr
                                AND belnr   =  bsip-belnr
                                AND buzei   =  bsip-buzei
                                AND xstov   = '' ORDER BY PRIMARY KEY.
...
* NEW CODE
        SELECT *
        UP TO 1 ROWS  FROM bsak WHERE  bukrs =  bukrs
                          AND belnr =  bsip-belnr
                          AND gjahr =  bsip-gjahr
                          AND buzei =  bsip-buzei
                          AND xstov = '' ORDER BY PRIMARY KEY.
...
* NEW CODE
          SELECT *
          UP TO 1 ROWS  FROM  reguh WHERE zbukr = bsak-bukrs
                                      AND   vblnr = bsak-augbl
                                      AND   zaldt = bsak-augdt ORDER BY PRIMARY KEY.
...
* NEW CODE
              SELECT *
              UP TO 1 ROWS  FROM payr WHERE zbukr = bsak-bukrs
                                        AND   vblnr = bsak-augbl
