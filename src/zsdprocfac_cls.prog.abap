...
* NEW CODE
          SELECT *
          UP TO 1 ROWS  INTO tmp_zcabpedext
            FROM zcabpedext
            WHERE znum_doc_core = ls_outtab-znum_doc_core
              AND zblart        = ls_outtab-zblart ORDER BY PRIMARY KEY.
...
* NEW CODE
              ...
              UP TO 1 ROWS  INTO bkpf-belnr
                FROM bkpf
                WHERE awtyp EQ gv_vbrk
                  AND awkey EQ ls_outtab-factura ORDER BY PRIMARY KEY.
...
* NEW CODE
              ...
              UP TO 1 ROWS 
                ...
                WHERE bukrs = wa_bkpf-bukrs
                  AND belnr = wa_bkpf-belnr
                  AND gjahr = wa_bkpf-gjahr ORDER BY PRIMARY KEY.
...
* NEW CODE
 ...
 FROM nast
              INTO CORRESPONDING FIELDS OF TABLE tmp_nast
              WHERE kappl EQ 'V3'
                AND objky EQ vbrk-vbeln ORDER BY PRIMARY KEY.
