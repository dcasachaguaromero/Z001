...
* NEW CODE
  SELECT *
 FROM FEBKO INTO TABLE YFEBKO
           WHERE KUKEY IN R_KUKEY ORDER BY PRIMARY KEY.
...
* NEW CODE
    SELECT *
 FROM FEBEP INTO TABLE YFEBEP
             WHERE KUKEY = FEBKO-KUKEY ORDER BY PRIMARY KEY.
...
* NEW CODE
                  SELECT LAND1
                  UP TO 1 ROWS  FROM T001 INTO l_country
...
* NEW CODE
      SELECT LAND1
      UP TO 1 ROWS  FROM T001 INTO HLP_LAND
                        WHERE BUKRS = FEBKO-BUKRS ORDER BY PRIMARY KEY.
...
* NEW CODE
 ...
 FROM BKPF WHERE BUKRS = FEBKO-BUKRS
                         AND BSTAT = ' '
                         AND XBLNR = TREFFER-NUMMER ORDER BY PRIMARY KEY.
...
* NEW CODE
 ...
 FROM BSEG WHERE BUKRS = BKPF-BUKRS
                           ...
                           AND AUGBL = SPACE
                           AND ( KOART = 'D'
                              OR KOART = 'K' ) ORDER BY PRIMARY KEY.
...
* NEW CODE
    SELECT *
 FROM BKPF WHERE BUKRS = FEBKO-BUKRS
                         AND BELNR = TREFFER-NUMMER ORDER BY PRIMARY KEY.
...
* NEW CODE
 ...
 FROM BKPF WHERE BUKRS = FEBKO-BUKRS
                           AND AWTYP = 'VBRK'
                           AND AWKEY = TREFFER-NUMMER ORDER BY PRIMARY KEY.
...
* NEW CODE
  SELECT *
 FROM PAYR WHERE ICHEC = SPACE
                     AND   ZBUKR = FEBKO-BUKRS
                     AND   HBKID = FEBKO-HBKID
                     AND   HKTID = FEBKO-HKTID
                     AND   CHECT = CHECT ORDER BY PRIMARY KEY.
...
* NEW CODE
      SELECT *
      UP TO 1 ROWS  FROM BKPF WHERE BUKRS = FEBKO-BUKRS
                                  AND BELNR = PAYR-VBLNR
...
* NEW CODE
      SELECT *
 FROM BKPF WHERE BUKRS = FEBKO-BUKRS          "hw357507
                            AND BELNR = L_BELNR ORDER BY PRIMARY KEY.            

* END. 07-07-2026 - ATC - ATC-03"hw357507
      ENDSELECT.                                            "hw357507
...
* NEW CODE
    SELECT *
 FROM BKPF WHERE BUKRS = FEBKO-BUKRS
                         AND BELNR = CHECT ORDER BY PRIMARY KEY.
...
* NEW CODE
  SELECT *
 FROM BKPF WHERE BUKRS = FEBKO-BUKRS              "ak051199
                       AND BELNR = CHECT ORDER BY PRIMARY KEY.                   

* END. 07-07-2026 - ATC - ATC-03"ak051199
...
* NEW CODE
  SELECT *
 FROM BKPF WHERE BUKRS = FEBKO-BUKRS
                       AND XBLNR = CHECT ORDER BY PRIMARY KEY.
...
* NEW CODE
        SELECT *
        UP TO 1 ROWS  FROM KNB1 WHERE KUNNR = AGKON
...
* NEW CODE
        SELECT *
        UP TO 1 ROWS  FROM LFB1 WHERE LIFNR = AGKON
                                    AND BUKRS = FEBKO-BUKRS ORDER BY PRIMARY KEY.
...
* BEGIN. 07-07-2026 - ATC - ATC-03
* OLD CODE
*  SELECT * FROM (DBTABLE) INTO <H_RESULT> WHERE (CONDTAB).     
*
* NEW CODE
  SELECT *
 FROM (DBTABLE) INTO <H_RESULT> WHERE (CONDTAB) ORDER BY PRIMARY KEY.     

* END. 07-07-2026 - ATC - ATC-03"UC
...
* NEW CODE
    SELECT LAND1
    UP TO 1 ROWS  FROM T001 INTO HLP_LAND             "note398160
                        WHERE BUKRS = FEBKO-BUKRS ORDER BY PRIMARY KEY.          

    ENDSELECT.
* END. 07-07-2026 - ATC - ATC-01"note398160
...
* NEW CODE
    SELECT *
    UP TO 1 ROWS  FROM PYORDH WHERE PYORD = TREFFER-NUMMER ORDER BY PRIMARY KEY.
...
* NEW CODE
  SELECT *
 INTO TABLE I_AVIK FROM AVIK
         WHERE  BUKRS      = P_BUKRS
         AND    VGREF      = P_CHECT
         AND    PAKTO      = P_HKONT ORDER BY PRIMARY KEY.
...
*
* NEW CODE
    SELECT * APPENDING TABLE YAVIK
 FROM AVIK
             WHERE AVSID = TREFFER-NUMMER ORDER BY PRIMARY KEY.
...
* NEW CODE
 ...
 from pyordp
                 ...
                 and    belnr  = h_bsid-belnr               "hw583577
                 and    gjahr  = h_bsid-gjahr               "hw583577
                 and    buzei  = h_bsid-buzei ORDER BY PRIMARY KEY.              

* END. 07-07-2026 - ATC - ATC-03"hw583577
* BEGIN. 07-07-2026 - ATC - ATC-01
* OLD CODE
...
* NEW CODE
          SELECT *
          UP TO 1 ROWS  from pyordh where pyord = pyordp-pyord ORDER BY PRIMARY KEY.
...
* NEW CODE
 ...
 from pyordp
                 ...
                 and    belnr  = h_bsik-belnr               "hw583577
                 and    gjahr  = h_bsik-gjahr               "hw583577
                 and    buzei  = h_bsik-buzei ORDER BY PRIMARY KEY.              
...
* NEW CODE
    SELECT LAND1
    UP TO 1 ROWS  FROM T001 INTO HLP_LAND             "note 398160
                        WHERE BUKRS = FEBKO-BUKRS ORDER BY PRIMARY KEY.          

    ENDSELECT.
* END. 07-07-2026 - ATC - ATC-01"note 398160
...
* NEW CODE
    SELECT *
 FROM KNBK WHERE BANKN = FEBEP-PAKTO ORDER BY PRIMARY KEY.
...
* NEW CODE
    SELECT *
 FROM KNBK WHERE BANKL = FEBEP-PABLZ
                         AND BANKN = FEBEP-PAKTO ORDER BY PRIMARY KEY.
...
* NEW CODE
      SELECT *
 FROM LFBK WHERE BANKN = FEBEP-PAKTO ORDER BY PRIMARY KEY.
...
* NEW CODE
      SELECT *
 FROM LFBK WHERE BANKL = FEBEP-PABLZ
                           AND BANKN = FEBEP-PAKTO ORDER BY PRIMARY KEY.
...
* NEW CODE
      SELECT *
      UP TO 1 ROWS  FROM KNA1 WHERE KUNNR = KNBK-KUNNR ORDER BY PRIMARY KEY.
...
* END. 07-07-2026 - ATC - ATC-03"HP31I
    ENDSELECT.                                              "HP31I
  ELSE.                                                     "HP31I
* BEGIN. 07-07-2026 - ATC - ATC-03
* OLD CODE
...
* NEW CODE
      SELECT *
      UP TO 1 ROWS  FROM LFA1 WHERE LIFNR = LFBK-LIFNR ORDER BY PRIMARY KEY.
