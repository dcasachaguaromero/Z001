...
* Valida numero KEY
  IF p_ti_cabecera-zkey  EQ space.
    p_t_error = 4.
    return-number            = '1'.
  ...
  v_RLDNR = p_ti_cabecera-ACC_PRINCIPLE+0(2).
...
** V1 RVY 05-03-24
  ...
  IF p_ti_cabecera-area_contab NE space.

* BEGIN. 07-07-2026 - ATC - ATC-01
* OLD CODE
*    SELECT SINGLE  prefix_txt FROM tsad4 INTO  i_zzcod_unidad
*       WHERE prefix_key =  p_ti_cabecera-area_contab.
*
* NEW CODE
    SELECT prefix_txt
    UP TO 1 ROWS  FROM tsad4 INTO  i_zzcod_unidad
       WHERE prefix_key =  p_ti_cabecera-area_contab ORDER BY PRIMARY KEY.
...
* END. 07-07-2026 - ATC - ATC-01
    IF sy-subrc NE 0.
      ...
      return-message_v3        = 'Numero de comprobante Externo '.
      return-message_v4        = p_ti_cabecera-area_contab.
      APPEND return.
    ENDIF.
  ENDIF.

  IF p_ti_cabecera-ACC_PRINCIPLE NE space.

    IF p_ti_cabecera-ACC_PRINCIPLE = '0L' OR
       p_ti_cabecera-ACC_PRINCIPLE = '1F' OR
       p_ti_cabecera-ACC_PRINCIPLE = '1S' OR
       p_ti_cabecera-ACC_PRINCIPLE = '1T'.
       sy-subrc = 0.
    else.
       sy-subrc = 1.
       p_t_error = 4.
       ...
       return-type             = 'E'.
       return-id                = '07'.
       return-message_v1        = 'Ledger no Valido'.
       return-message_v2        = ' '.
       return-message_v3        = ' '.
       return-message_v4        = p_ti_cabecera-ref_doc_no.
       APPEND return.
...
FORM val_detalle  TABLES  return STRUCTURE bapiret2
                  USING    p_ti_detalle  STRUCTURE zdetalle_me
...
*&---------------------------------------------------------------------*
FORM contabilizacion.
  DATA: lv_bankn  LIKE lfbk-bankn.
  DATA: lv_string TYPE c LENGTH 37.
...
* Estructura de Posición Acreedor
      ...
      "mod contabilizacion. Seidor Crystalis 10.12.2013
      ...
      IF ti_cont_det-wt_type <> space AND
        ...
        IF ( ti_cont_det-wt_type = 'X' AND ti_cont_det-wt_code  = 'X' ).
          ...
          IF  ti_cont_cab-doc_date(4) = 2021." agrego 2021 HCD
...
* Ingreso de montos para la segunda moneda.
  currencyamount-itemno_acc = ti_cont_det-itemno_acc.
  currencyamount-curr_type  = '10'.
  currencyamount-amt_doccur = ti_cont_det-amt_doccur_me.
  currencyamount-currency   = ti_cont_det-currency_me.
...
* Traspaso datos de ejecucion de bapi a estructura de resumen.
  IF error EQ 4.
    MOVE: ti_cont_cab-zkey TO  p_ti_resumen-key,
...
FORM limpia_tablas .
  REFRESH: return,
           ...
           t_mwdat,
           ti_cont_det,
           ti_cont_det_ct,  "ECDK923606  V1-CNN: Habilitación nueva moneda SAP
           ti_cont_cab,
           ti_error_det,
           ti_error_det_ct, "ECDK923606  V1-CNN: Habilitación nueva moneda SAP
           ti_error_cab.
ENDFORM.
