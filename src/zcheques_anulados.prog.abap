...
SELECT *
 from payr
  where zbukr in s_zbukr
        and voidr ne 0 ORDER BY PRIMARY KEY.
...
* BEGIN. 07-07-2026 - ATC - ATC-01
* OLD CODE
*  select SINGLE BLDAT BUDAT STBLG STJAH
*    into (it_spins-pfec_doc, it_spins-pfec_cont, w_belnr, w_gjahr)
*    from bkpf
...
*
* NEW CODE
  SELECT BLDAT BUDAT STBLG STJAH
  UP TO 1 ROWS 
    into (it_spins-pfec_doc, it_spins-pfec_cont, w_belnr, w_gjahr)
    from bkpf
    where bukrs = payr-zbukr
...
* NEW CODE
  ...
  ENDSELECT.
* END. 07-07-2026 - ATC - ATC-01
  IF w_belnr ne ' ' and w_belnr is not initial.
    it_spins-pnro_doc = w_belnr.
  ELSE.
    it_spins-pnro_doc = payr-vblnr.
  ENDIF.
  IF w_gjahr ne ' ' and w_gjahr is not initial.
    it_spins-pejercicio = w_gjahr.
  ELSE.
    it_spins-pejercicio = payr-gjahr.
  ENDIF.
  it_spins-pfec_anula = payr-voidd. "Fecha de Anulacion del cheque
...
*         execute procedure pkg_sap_cargas.sap_carga_detalle_gasto ( IN :hkont )
  ...
  loop at t_spins INTO it_spins.
    try.
      ...
      EXEC SQL.
        EXECUTE PROCEDURE csc_sap_auxiliares.sp_insert_anulados(
