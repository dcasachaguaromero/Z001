...
* NEW CODE
      ...
      UP TO 1 ROWS  INTO zcambiocheque-xblnr
        FROM zcambiocheque
        WHERE xblnr = v_xblnr ORDER BY PRIMARY KEY.
...
* NEW CODE
  SELECT *
 INTO CORRESPONDING FIELDS OF TABLE  ti_zrever
    FROM zreversacheque
   WHERE bukrs  EQ p_bukrs
     AND hbkid  EQ p_hbkid
     AND hktid  EQ p_hktid
     AND estrever EQ 'N'
     AND chect  EQ  p_chect
     AND hkontd EQ  p_hkont ORDER BY PRIMARY KEY.
