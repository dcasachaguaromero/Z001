...
*****Get Item/Header conditions for contract *************
CALL FUNCTION 'ME_RESET_CONTRACT_CONDITIONS'.

CALL FUNCTION 'ME_GET_CONTRACT_CONDITIONS'
EXPORTING
beleg   = is_ekko-ebeln
schema  = is_ekko-kalsm
sprache = gv_language.

DATA ls_ekpo2   TYPE ekpo.
DATA lt_ekomd_i TYPE TABLE OF ekomd.

LOOP AT it_ekpo INTO ls_ekpo2.
REFRESH lt_ekomd_i.
CALL FUNCTION 'ME_PRINT_CONTRACT_CONDITIONS'
EXPORTING
position = ls_ekpo2-ebelp
datum    = is_ekko-kdate
TABLES
tekomd   = lt_ekomd_i.
APPEND LINES OF lt_ekomd_i TO gt_ekomd.
ENDLOOP.
...
*****End of getting Item/Header conditions for contract **
