...
* NEW CODE
 ...
 INTO CORRESPONDING FIELDS OF TABLE lt_t087v
         FROM t087v WHERE spras EQ sy-langu
                      AND anlue IN s_anlue ORDER BY PRIMARY KEY.

* END. 07-07-2026 - ATC - ATC-03"#EC CI_SGLSELECT
**add comment fin
* BEGIN. 07-07-2026 - ATC - ATC-03
* OLD CODE
...
* NEW CODE
    SELECT lifnr land1 name1 stcd1 APPENDING TABLE gt_lifnr
           ...
           FROM lfa1 WHERE lifnr EQ wa_salida-lifnr ORDER BY PRIMARY KEY.
...
*
* NEW CODE
        SELECT blart bktxt xref1_hd xref2_hd waers
        UP TO 1 ROWS 
               INTO (wa_datos-blart, wa_datos-bktxt,
                     wa_datos-xref1_hd, wa_datos-xref2_hd,
                     wa_datos-waers )
               FROM bkpf WHERE bukrs EQ wa_datos-bukrs
                           AND belnr EQ wa_datos-belnr
                           AND gjahr EQ wa_datos-gjahr ORDER BY PRIMARY KEY.
...
*
* NEW CODE
        SELECT belnr blart bktxt xref1_hd xref2_hd waers
        UP TO 1 ROWS 
               INTO (wa_datos-belnr_fi, wa_datos-blart_fi, wa_datos-bktxt,
                     wa_datos-xref1_hd, wa_datos-xref2_hd,
                     wa_datos-waers )
               FROM bkpf WHERE bukrs EQ wa_datos-bukrs
                           AND awtyp EQ lw_anek-awtyp
                           AND awkey EQ lv_awkey ORDER BY PRIMARY KEY.
...
* NEW CODE
  SELECT bwasl anbtr
  UP TO 1 ROWS  INTO (s_bwasl, s_anbtr )
         FROM anep WHERE bukrs  EQ p_lw_anek-bukrs
                     AND anln1  EQ p_lw_anek-anln1
                     AND anln2  EQ p_lw_anek-anln2
                     AND gjahr  EQ p_lw_anek-gjahr
                     AND lnran  EQ p_lw_anek-lnran
                     AND afabe  EQ wa_salida-afabe
                     AND lnsan  EQ space ORDER BY PRIMARY KEY.
