...
* NEW CODE
  SELECT *
 INTO *nast FROM nast WHERE kappl = nast-kappl
                                AND   objky = nast-objky
                                AND   kschl = nast-kschl
                                AND   spras = nast-spras
                                AND   parnr = nast-parnr
                                AND   parvw = nast-parvw
                                AND   nacha BETWEEN '1' AND '4' ORDER BY PRIMARY KEY.
...
* NEW CODE
  SELECT *
  UP TO 1 ROWS  FROM tvko  WHERE vkorg = vbdkr-vkorg ORDER BY PRIMARY KEY.
...
* NEW CODE
    SELECT *
    UP TO 1 ROWS  FROM t001g WHERE bukrs = vbdkr-bukrs
                                 AND programm EQ sy-repid
                                 AND txtid EQ 'SD' ORDER BY PRIMARY KEY.
...
* NEW CODE
  SELECT *
  UP TO 1 ROWS  FROM vbrk WHERE vbeln EQ nast-objky ORDER BY PRIMARY KEY.
...
* NEW CODE
    SELECT *
    UP TO 1 ROWS  FROM ztdea WHERE blart EQ vbrk-zblart
...
* NEW CODE
      SELECT adrnr
      UP TO 1 ROWS   INTO t001-adrnr
        FROM t001  WHERE bukrs EQ vbrk-bukrs ORDER BY PRIMARY KEY.
...
* NEW CODE
        SELECT *
        UP TO 1 ROWS  FROM adrc WHERE addrnumber EQ t001-adrnr ORDER BY PRIMARY KEY.
...
* NEW CODE
      ...
      ENDSELECT.
* END. 07-07-2026 - ATC - ATC-01"Pedido
      IF sy-subrc = 0.
* BEGIN. 07-07-2026 - ATC - ATC-01
* OLD CODE
...
* NEW CODE
        ...
        UP TO 1 ROWS  INTO wa_zcabpedext
          FROM zcabpedext WHERE pedido EQ vbfa-vbelv ORDER BY PRIMARY KEY.
...
*       AND posnv   = wa_vbrp-vgpos
        AND vbtyp_n = 'M'
        AND vbtyp_v = 'C' ORDER BY PRIMARY KEY.
...
* NEW CODE
      SELECT *
      UP TO 1 ROWS  FROM vbrk WHERE vbeln  = <ls_vbfa>-vbeln
                                  AND fksto <> 'X'
                                  AND rfbsk  = 'C' ORDER BY PRIMARY KEY.
...
* NEW CODE
  SELECT kunnr parvw
 INTO (l_kunnr, l_parvw )
         FROM vbpa WHERE vbeln EQ vbrk-vbeln AND
                         parvw IN ('RE','RG') ORDER BY PRIMARY KEY.
...
* NEW CODE
    SELECT stcd1
    UP TO 1 ROWS  INTO wa_zcabpedext-zrut_cli_fact
         FROM kna1 WHERE kunnr EQ l_kunnr_re ORDER BY PRIMARY KEY.
...
*
* NEW CODE
    SELECT stcd1 name1 stras ort02
    UP TO 1 ROWS 
      INTO CORRESPONDING FIELDS OF ls_kna1
      FROM kna1 WHERE kunnr EQ l_kunnr_re ORDER BY PRIMARY KEY.
...
* NEW CODE
  SELECT *
  UP TO 1 ROWS  FROM zhostacepta WHERE sysid EQ sy-sysid ORDER BY PRIMARY KEY.
...
* NEW CODE
    ...
    UP TO 1 ROWS   INTO wa_zcabpedext
      FROM zcabpedext
      WHERE znum_doc_core = wa_zcabpedext-znum_doc_core ORDER BY PRIMARY KEY.
...
* NEW CODE
        SELECT *
        UP TO 1 ROWS  FROM vbrk INTO ls_vbrk WHERE vbeln = nast-objky ORDER BY PRIMARY KEY.
...
* NEW CODE
          SELECT *
          UP TO 1 ROWS  FROM bkpf WHERE bukrs = ls_vbrk-bukrs
                                      AND awtyp = c_vbrk
                                      AND awkey = ls_vbrk-vbeln ORDER BY PRIMARY KEY.
...
* NEW CODE
    ...
    UP TO 1 ROWS  INTO wa_zcabpedext
      FROM zcabpedext
      WHERE znum_doc_core EQ wa_zcabpedext-znum_doc_core ORDER BY PRIMARY KEY.
