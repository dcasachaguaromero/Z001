...
* NEW CODE
  ...
  UP TO 1 ROWS  FROM T001
                INTO (GS_T001-BUKRS, GS_T001-BUTXT, GS_T001-WAERS,
                      GS_T001-ADRNR)
                WHERE BUKRS = SD_BUKRS-LOW ORDER BY PRIMARY KEY.
...
*
* NEW CODE
  SELECT BUKRS BELNR GJAHR BUDAT LDGRP BSTAT
 FROM BKPF
          INTO TABLE GT_BKPF
          WHERE BUKRS = SD_BUKRS-LOW
          AND   GJAHR = SD_GJAHR-LOW
*          AND BSTAT EQ SPACE              " note 935448
          AND   BUDAT IN R_DATE ORDER BY PRIMARY KEY.
...
* NEW CODE
     SELECT *
 FROM FAGLFLEXT INTO TABLE LT_FAGLFLEXT
        WHERE RBUKRS = SD_BUKRS-LOW and
              RYEAR  = SD_GJAHR-LOW and
              RACCT  = GS_OUTPUT-SAKNR and
              RLDNR  = SD_RLDNR-LOW ORDER BY PRIMARY KEY.
...
* NEW CODE
  SELECT WAERS
 INTO LV_WAERS FROM T001 WHERE BUKRS = SD_BUKRS-LOW ORDER BY PRIMARY KEY.
...
* NEW CODE
 ...
 FROM ZFIGIRO
    INTO TABLE TI_ZFIGIRO
    WHERE BUKRS = GS_T001-BUKRS ORDER BY PRIMARY KEY.
