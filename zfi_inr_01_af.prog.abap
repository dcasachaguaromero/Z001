*&---------------------------------------------------------------------*
*& Report  ZFI_INR_01_AF
*&---------------------------------------------------------------------*
*& ISAP --> M. Encina V.
*& Cálculo Iva proporcional C9.
*& PGR Test : Agrega Logica p/ activos Fijos
*&---------------------------------------------------------------------*
REPORT  zfi_inr_01_af NO STANDARD PAGE HEADING.

INCLUDE: zfi_inr_01_inc_af,
         zfi_inc_bi    .
...
DATA: g_bdcop     LIKE ctu_params,
      ...
      g_sgtxt(30) TYPE c,
      g_monto(10) TYPE c,
      g_hkont(10) TYPE c VALUE '1013310005',
...
*SELECT-OPTIONS  s_mwskz FOR  bseg-mwskz DEFAULT 'C9' OBLIGATORY.
...
**********************************************************************
* AGREGA RUTINAS PARA MIRO CON ACTIVOS FIJOS
**********************************************************************
  CLEAR:   it_bkpf,
             it_rbkp,
             it_rseg,
...
***** Dcumentos TCODE = MIRO , DOC. F4   "
*     VALIDAR USUARIOS SI USAN OTRAS CLASES DOC.
...
* NEW CODE
 ...
 INTO TABLE it_bseg
     ...
     WHERE bukrs    EQ it_bkpf-bukrs   AND
           belnr    EQ it_bkpf-belnr   AND
           gjahr    EQ it_bkpf-gjahr   AND
           ktosl    EQ 'ANL' ORDER BY PRIMARY KEY. 

* END. 07-07-2026 - ATC - ATC-03"Pos de AF
...
*       LEVANTA POSICIÓN DE DOC. COMPRA
* BEGIN. 07-07-2026 - ATC - ATC-01
* OLD CODE
*          SELECT SINGLE * FROM rseg
*            WHERE belnr  EQ it_bkpf-awkey(10) AND
*                  gjahr  EQ it_bkpf-gjahr AND
...
*               rseg-shkzg       TO t_alv-shkzg,
                  ...
                  it_bseg-anln1    TO t_alv-anln1, "Activo Fijo
                  it_bseg-anln2    TO t_alv-anln2, "Subnumero
                  it_bkpf-xblnr    TO t_alv-referencia. "Nuevo campo referencia GCR

            IF t_alv-shkzg = 'S'. "Valor al Debe
              t_alv-dmbtr = t_alv-dmbtr * -1.
            ENDIF.

* PONEMOS LOS DATOS DESDE LA MISMA POSICIÓN
            MOVE: it_bseg-hkont      TO t_alv-sakto      , "Cuenta Mayor
                  it_bseg-kostl      TO t_alv-kostl      , "Ceco
                  it_bseg-zzunid_pro TO t_alv-zzunid_pro , "Cód Unidad y Cód de Producto
                  it_bseg-zzrut_terc TO t_alv-rut_terc   . "nuevo campo de despligue gcr
...
*     VALIDA TIPO DE CUENTA, DEBE SER 1 PARA ACTIVO FIJO

            IF t_alv-sakto+0(1) NE '1'.
              CLEAR  t_alv.
            ELSE.
*           Fecha de contab. menor igual a fecha maxima select.
              IF bkpf-budat <= s_budat-high. "Contorl de Fechas
                APPEND t_alv.
              ELSE.
                CLEAR  t_alv.
              ENDIF.
          ...
          ELSE.
            MOVE 1 TO sw_miro. "No encontro IVA C9
          ENDIF.
        ENDLOOP.
      ENDLOOP.
    ELSE.
      MOVE 1 TO sw_miro. "No encontro Posiciones
    ENDIF.
  ELSE.
    MOVE 1 TO sw_miro. "No encontro Documentos
  ENDIF.
*
******************************************************************
*  FIN AGREGA RUTINA PARA MIRO Y AF
*******************************************************************
*
*

  IF sw_miro = 1.
    MESSAGE 'Sin seleción de datos, verifique parámetros' TYPE 'E'.
  ENDIF.


  LOOP AT t_alv.

* Calcula IVA de posición
...
* END. 07-07-2026 - ATC - ATC-01
** V1 RVY 26-01-2021
* Calcula valor Iva Proporcional
    IF sy-subrc  = 0.
...
*             sy-datum+00(04) INTO g_fecha.
  ...
  CONCATENATE t_alv-anln1 t_alv-anln2 INTO g_activ SEPARATED BY '-'.
