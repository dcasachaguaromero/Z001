...
* NEW CODE
    SELECT *
 FROM tpakd WHERE parvw = auftraggeber ORDER BY PRIMARY KEY.
...
* NEW CODE
    SELECT *
 FROM t077d WHERE ktokd IN rktokd
                        AND   xcpds = 'X' ORDER BY PRIMARY KEY.
...
* NEW CODE
  SELECT *
  UP TO 1 ROWS  FROM tvkov WHERE vkorg = s_vkorg
                             AND   vtweg = s_vtweg ORDER BY PRIMARY KEY.
...
* NEW CODE
  SELECT *
  UP TO 1 ROWS  FROM tvkos WHERE vkorg = s_vkorg
                             AND   spart = s_spart ORDER BY PRIMARY KEY.
...
* NEW CODE
  SELECT maind
  UP TO 1 ROWS  INTO lv_maind FROM tvind WHERE trvog EQ '6' ORDER BY PRIMARY KEY.
...
* NEW CODE
      ...
      FROM lips WHERE   matnr IN matnr
                AND     charg IN charge
                AND NOT xchpf EQ ' ' ORDER BY PRIMARY KEY.
