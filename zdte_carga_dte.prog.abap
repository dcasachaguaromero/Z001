...
*  WHERE estado  EQ c_nuevo
  WHERE estado  IN s_estado   "nuevo y no contab.
    AND tipodte IN s_tipodte ORDER BY PRIMARY KEY. 
...
* NEW CODE
      ...
      UP TO 1 ROWS  INTO w_t001z
        FROM t001z
       WHERE paval EQ wa_doc_rec-rutrecep ORDER BY PRIMARY KEY.
...
* NEW CODE
      SELECT *
      UP TO 1 ROWS  INTO w_cierre_cont
        FROM zdte_cierre_cont
       WHERE bukrs EQ w_t001z-bukrs
         AND gjahr EQ sy-datum(4)
         AND monat EQ sy-datum+4(2) ORDER BY PRIMARY KEY.
...
* NEW CODE
      SELECT *
      UP TO 1 ROWS   INTO wa_listablanca
      FROM zbmnc_listabcab
      WHERE numero_cliente EQ wa_doc_rec-numero_cliente
        AND rut_proveedor  EQ wa_doc_rec-rutemisor ORDER BY PRIMARY KEY.
...
* NEW CODE
      ...
      UP TO 1 ROWS 
        FROM zdte_tabla_doc
       WHERe bukrs     EQ w_t001z-bukrs
         AND rutemisor EQ wa_doc_rec-rutemisor ORDER BY PRIMARY KEY.
...
* NEW CODE
        SELECT *
 INTO CORRESPONDING FIELDS OF TABLE ti_val_dte
          FROM zmm_val_dte
         WHERE bukrs = lv_bukrs ORDER BY PRIMARY KEY.
...
* NEW CODE
  SELECT *
  UP TO 1 ROWS  INTO w_lfa1
    FROM lfa1
    WHERE stcd1 EQ  p_rut ORDER BY PRIMARY KEY.
...
* NEW CODE
    SELECT *
    UP TO 1 ROWS  INTO w_bseg_ref
      FROM bseg
     WHERE bukrs EQ w_bkpf-bukrs
       AND belnr EQ w_bkpf-belnr
       AND gjahr EQ w_bkpf-gjahr
       AND bschl EQ '86' ORDER BY PRIMARY KEY.
...
* NEW CODE
      SELECT *
      UP TO 1 ROWS  INTO w_bseg_doc
        FROM bseg
       WHERE bukrs EQ w_bkpf-bukrs
*         AND belnr EQ w_bkpf-belnr
         AND gjahr EQ w_bkpf-gjahr
         AND xref3 EQ w_bseg_ref-xref3
         AND bschl EQ '96' ORDER BY PRIMARY KEY.
