...
* NEW CODE
  ...
  WHERE rutemisor EQ wa_data-rutemisor
    AND tipodte   EQ wa_data-tipodte
    AND folio     EQ wa_data-folio ORDER BY PRIMARY KEY.
...
* NEW CODE
  SELECT bukrs party
  UP TO 1 ROWS  INTO (lv_bukrs, lv_party)
         FROM   t001z
         WHERE  paval = wa_data-rutrecep ORDER BY PRIMARY KEY.
...
* NEW CODE
    SELECT butxt adrnr
    UP TO 1 ROWS  INTO (lv_butxt, lv_adrnr)
           FROM   t001
...
*
* NEW CODE
      SELECT city1 street house_num1
      UP TO 1 ROWS 
        INTO (lv_city1, lv_street, lv_house_num1)
        FROM adrc
...
* NEW CODE
    ...
    UP TO 1 ROWS 
    FROM lfa1
    WHERE stcd1 EQ wa_data-rutemisor ORDER BY PRIMARY KEY.
...
* NEW CODE
      ...
      ENDSELECT.
* END. 07-07-2026 - ATC - ATC-01"Verificar Sociedad?
      IF sy-subrc EQ 0.
        "Nada.
      ELSE.
...
*
* NEW CODE
    SELECT *  "Validamos Factura de Servicio
    UP TO 1 ROWS 
    FROM ekpo
    WHERE ebeln EQ vl_ebeln
      AND ebelp EQ 10
      AND pstyp EQ '9' ORDER BY PRIMARY KEY.
...
* NEW CODE
        ...
        FROM ekbe
        WHERE ebeln EQ vl_ebeln
          AND belnr EQ vl_hes ORDER BY PRIMARY KEY.
