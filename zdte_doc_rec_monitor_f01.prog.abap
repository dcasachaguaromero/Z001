...
* NEW CODE
  SELECT lifnr
  UP TO 1 ROWS  INTO p_lifnr  FROM  lfa1
           WHERE stcd1  = p_rut ORDER BY PRIMARY KEY.
...
* NEW CODE
  SELECT lifnr
  UP TO 1 ROWS  INTO l_lifnr  FROM  lfa1
         WHERE stcd1  = wa_doc_rec-rutemisor ORDER BY PRIMARY KEY.
...
* NEW CODE
  SELECT *
  UP TO 1 ROWS  INTO wa_t003 FROM  t003
         WHERE  blart  = 'ZE' ORDER BY PRIMARY KEY.
...
* NEW CODE
  SELECT zterm zwels
  UP TO 1 ROWS  INTO  (l_zterm,l_zwels) FROM lfb1 WHERE lifnr = l_lifnr AND bukrs = '0100' ORDER BY PRIMARY KEY.
...
* NEW CODE
    SELECT konth
    UP TO 1 ROWS  INTO l_konth FROM  t030k
           WHERE  ktopl  = 'PCPR'
           AND    ktosl  = 'VST' "'MWS'
           AND    mwskz  = 'C2' ORDER BY PRIMARY KEY.
...
* NEW CODE
  SELECT *
 INTO TABLE lt_ref  FROM  zdte_doc_rec_ref
         WHERE  rutemisor  = p_data-rutemisor
         AND    tipodte    = p_data-tipodte
         AND    folio      = p_data-folio ORDER BY PRIMARY KEY.
